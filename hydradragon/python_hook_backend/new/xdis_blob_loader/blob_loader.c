...
#define PYTHON_VERSION ((PY_MAJOR_VERSION << 8) | (PY_MINOR_VERSION << 4))
#define unlikely(x) (x)
...
#define Nuitka_PyUnicode_CheckConsistency(u, i) 1
#define builtin_module PyImport_ImportModule("builtins")
#define PRINT_FORMAT(...) printf(__VA_ARGS__)
#define PRINT_STRING(s) printf("%s", s)
#define NUITKA_CANNOT_GET_HERE(msg) { PyErr_SetString(PyExc_RuntimeError, msg); return NULL; }
#define MAKE_SLICE_OBJECT3(tstate, start, stop, step) PySlice_New(start, stop, step)
#define BUILTIN_XRANGE3(tstate, start, stop, step) (((void)tstate), PyObject_CallFunctionObjArgs((PyObject*)&PyRange_Type, start, stop, step, NULL))
#define BUILTIN_COMPLEX2(tstate, real, imag) (((void)tstate), PyComplex_FromDoubles(PyFloat_AsDouble(real), PyFloat_AsDouble(imag)))
#define MAKE_UNION_TYPE(args) (((void)args), PyDict_New()) // dummy
#define CALL_FUNCTION_WITH_SINGLE_ARG(tstate, func, arg) (((void)tstate), PyObject_CallFunctionObjArgs(func, arg, NULL))

#define Nuitka_PyUnion_Type ((PyObject*)&PyType_Type)
#define Py_SysVersionInfo PySys_GetObject("version_info")
#define _PyUnicode_InternImmortal(tstate, ptr) PyUnicode_InternInPlace(ptr)
#define NUITKA_STATIC_SMALLINT_VALUE_MIN -5
#define NUITKA_STATIC_SMALLINT_VALUE_MAX 256
#ifndef PyFloat_SET_DOUBLE
#define PyFloat_SET_DOUBLE(obj, val) (((PyFloatObject *)(obj))->ob_fval = (val))
#endif
#ifndef PyUnion_Type
#define PyUnion_Type PyType_Type
#endif

static unsigned char const *_unpackBlobConstants(PyThreadState *tstate, PyObject **output, unsigned char const *data, int count);


// Simplified Check_Object
#define CHECK_OBJECT(x) 

#define NUITKA_DYNAMIC_ARRAY_DECL(name, type, size) type *name = (type *)malloc(size * sizeof(type))

// Mock Py_INCREF_IMMORTAL
#ifndef Py_INCREF_IMMORTAL
#define Py_INCREF_IMMORTAL(x) Py_INCREF(x)
#endif
#ifndef Py_SET_REFCNT_IMMORTAL
#define Py_SET_REFCNT_IMMORTAL(x) Py_INCREF(x)
#endif

static void Nuitka_LongSetSignNegative(PyObject *obj) {
    // This is problematic in generic C Python because objects are immutable.
    // In our payload, we don't really use this much, or we can just ignore it for now or do a slow multiply?
    // Actually `Nuitka_LongSetSignNegative` in breakpoint.c is used like:
    // `if (c == 'G') Nuitka_LongSetSignNegative(result);`
    // We can't do it in-place easily via public C-API. We will mock it as a no-op 
    // or just let it break if used (it's rarely used).
}

// Pseudo MAKE_CODE_OBJECT that creates a dict
static PyObject* MAKE_CODE_OBJECT(PyObject *filename, int line, int flags, PyObject *function_name,
                               PyObject *function_qualname, PyObject *argnames, PyObject *freevars, int arg_count,
                               int kw_only_count, int pos_only_count) {
    PyObject* dict = PyDict_New();
    PyDict_SetItemString(dict, "type", PyUnicode_FromString("code"));
    PyDict_SetItemString(dict, "filename", filename ? filename : Py_None);
    PyDict_SetItemString(dict, "line", PyLong_FromLong(line));
    PyDict_SetItemString(dict, "flags", PyLong_FromLong(flags));
    PyDict_SetItemString(dict, "function_name", function_name ? function_name : Py_None);
    PyDict_SetItemString(dict, "function_qualname", function_qualname ? function_qualname : Py_None);
    PyDict_SetItemString(dict, "argnames", argnames ? argnames : Py_None);
    PyDict_SetItemString(dict, "freevars", freevars ? freevars : Py_None);
    PyDict_SetItemString(dict, "arg_count", PyLong_FromLong(arg_count));
    PyDict_SetItemString(dict, "kw_only_count", PyLong_FromLong(kw_only_count));
    PyDict_SetItemString(dict, "pos_only_count", PyLong_FromLong(pos_only_count));
    return dict;
}

static uint32_t calcCRC32(const unsigned char *data, size_t len) { return 0; } // dummy
...
static PyObject* decode_nuitka_blob(PyObject* self, PyObject* args) {
    ...
    // The entire blob structure: hash(4) + size(4) + [ name(null-term) + size(4) + blob... ]
    if (blob_size < 8) {
    ...
    while ((Py_ssize_t)(w - blob_data) < blob_size) {
        ...
        if (!output) { output = Py_None; Py_INCREF(Py_None); }
        
        PyDict_SetItemString(result_dict, name, output);
        Py_DECREF(output);
...
PyMODINIT_FUNC PyInit_xdis_blob_loader(void) {
