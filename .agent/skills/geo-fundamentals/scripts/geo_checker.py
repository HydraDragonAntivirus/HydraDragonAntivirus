...
SKIP_DIRS = {"node_modules", ".next", "dist", "build", ".git", ".github", "__pycache__", ".vscode", ".idea", "coverage", "test", "tests", "__tests__", "spec", "docs", "documentation"}

# Files to skip (not public pages)
SKIP_FILES = {"jest.config", "webpack.config", "vite.config", "tsconfig", "package.json", "package-lock", "yarn.lock", ".eslintrc", "tailwind.config", "postcss.config", "next.config"}
...
def is_page_file(file_path: Path) -> bool:
    ...
    page_indicators = ["page", "index", "home", "about", "contact", "blog", "post", "article", "product", "service", "landing"]
...
def check_page(file_path: Path) -> dict:
    ...
    entity_patterns = [r'"@type"\s*:\s*"Organization"', r'"@type"\s*:\s*"LocalBusiness"', r'"@type"\s*:\s*"Brand"', r"itemtype.*schema\.org/(Organization|Person|Brand)", r'rel="author"']
    ...
    direct_answer_patterns = [r"is defined as", r"refers to", r"means that", r"the answer is", r"in short,", r"simply put,", r"<dfn"]
...
def main():
    ...
    # JSON output
    output = {"script": "geo_checker", "project": str(target_path), "pages_checked": len(results), "average_score": round(avg_score), "passed": avg_score >= 60}
    print("\n" + json.dumps(output, indent=2))
