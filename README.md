...
- **Don't Run in Safe Mode:** HydraDragonAntivirus is not compatible with Safe Mode, and running it there is strongly discouraged. To prevent malware from abusing Safe Mode, HydraDragonAntivirus includes a Safe Mode protection rule that detects attempts to enable Safe Mode boot options, modify related registry settings, or otherwise force the system to boot into Safe Mode.
 ...
 2. **PPL-Enforced Communication**: All rules and management commands are injected via a secure IOCTL/Comms channel that strictly verifies the caller. Core security drivers (**Sanctum, OpenEDR, MBRFilter, SimplePYAS, and Owlyshield**) now refuse to accept commands unless the initiating process is a verified **Antimalware Protected Light (PPL)** process (`sanctum_ppl_runner.exe`). Since standard malware (even with Administrative privileges) cannot easily spoof or inject into a PPL process, this ensures that only the legitimate, hardened HydraDragon service can modify the security policy.
 3. **Zero-Trust Identity Enforcement**: The driver identifies the rule provider using standard Windows PPL identity checks (Protected Process Light with Antimalware signer) rather than brittle absolute paths, allowing for flexible deployment while maintaining cryptographically enforced identity. **Note**: Debugger components like **HyperDbg** and **RedDbg** are exempt from this PPL requirement to allow for normal initialization and analyst usage.
 4. **Early-Boot Resilience**: By moving rules from disk to memory, we mitigate attacks where **early-launched malware** attempts to delete or replace rule files before the AV service starts. However, in a "Post-Infection" scenario where malware has already achieved persistence, it may attempt to delete the AV binaries themselves. To defend against this, HydraDragon relies on **ELAM (Early Launch Anti-Malware)** and **PPL (Protected Process Lite)** to ensure the antivirus core initializes and protects itself before third-party malicious code can execute.
