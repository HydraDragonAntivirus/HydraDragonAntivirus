...
fn build_with_cmake() {
    ...
    if cfg!(feature = "dynamic_linkage") {
        ...
        config.define("UNICORN_LEGACY_STATIC_ARCHIVE", "ON");
    }
    ...
    if cfg!(feature = "dynamic_linkage") {
        if compiler.is_like_msvc() {
            ...
            // Copy unicorn.dll to the cargo output directory so the exe finds it at runtime
            let out_dir = std::env::var("OUT_DIR").unwrap();
            let target_dir = std::path::Path::new(&out_dir)
                .parent().unwrap()
                .parent().unwrap()
                .parent().unwrap();
            let dll_src = dst.join("bin").join("unicorn.dll");
            if dll_src.exists() {
                let dll_dst = target_dir.join("unicorn.dll");
                if !dll_dst.exists() {
