...
pub fn App() -> Html {
    ...
    let pe_sub_tab = use_state(|| "fields".to_string()); // "fields" | "sections" | "imports" | "exports"
    ...
    let on_open_file = {
        ...
        let entropy_sum = entropy_sum.clone();
        let extracted_strs = extracted_strs.clone();
    ...
    let on_jump = {
        let jump_offset = jump_offset.clone();
        let hex_offset = hex_offset.clone();
        let hex_page = hex_page.clone();
        let status = status.clone();
        Callback::from(move |_: MouseEvent| {
    ...
    let on_disasm = {
        ...
        let disasm_arch = disasm_arch.clone();
        let disasm_base = disasm_base.clone();
        let selected = selected_byte.clone();
        let hex_offset = hex_offset.clone();
        let status = status.clone();
        Callback::from(move |_: MouseEvent| {
            let disasm_rows = disasm_rows.clone();
            ...
            let base_addr = *disasm_base;
            let offset = selected.as_ref().copied().unwrap_or(*hex_offset);
            spawn_local(async move {
                status.set(format!("Disassembling at 0x{offset:X}…"));
                match invoke::<_, Vec<DisasmRow>>(
                    "disassemble_at",
                    &json!({"offset":offset,"arch":arch,"base_addr":base_addr,"num_insns":DISASM_INSNS}),
                )
                .await
                {
                    Ok(rows) => {
                        let n = rows.len();
                        disasm_rows.set(rows);
                        status.set(format!("Disassembled {n} instructions."));
                    }
            ...
            spawn_local(async move {
                ...
                match invoke::<_, Vec<YaraHit>>("scan_yara", &json!({"rules_source":rules})).await {
                    Ok(hits) => {
                        let n = hits.len();
                        yara_hits.set(hits);
                        status.set(format!("YARA: {n} matches."));
                        bottom_tab.set(BottomTab::YaraResults);
        ...
        Callback::from(move |_: MouseEvent| {
            ...
            spawn_local(async move {
                ...
                match invoke::<_, EntropySummary>("compute_entropy", &json!({"block_size":256})).await {
                    Ok(s) => {
                        entropy_sum.set(Some(s));
                        status.set("Entropy done.".into());
                        bottom_tab.set(BottomTab::Entropy);
                    }
    ...
    let on_extract_strings = {
        ...
        let status = status.clone();
        let bottom_tab = bottom_tab.clone();
        let str_min_len = str_min_len.clone();
        Callback::from(move |_: MouseEvent| {
            let extracted_strs = extracted_strs.clone();
            let status = status.clone();
            let bottom_tab = bottom_tab.clone();
            ...
            spawn_local(async move {
                ...
                match invoke::<_, Vec<ExtractedString>>("extract_strings", &json!({"min_len":min})).await
                {
                    Ok(ss) => {
                        let n = ss.len();
                        extracted_strs.set(ss);
                        status.set(format!("Strings: {n} found."));
                        bottom_tab.set(BottomTab::Strings);
                    }
        ...
        Callback::from(move |_: MouseEvent| {
            ...
            spawn_local(async move {
                ...
                match invoke::<_, ParsedHeaders>("parse_headers", &json!({})).await {
                    Ok(h) => {
                        parsed_headers.set(Some(h));
                        status.set("Headers parsed.".into());
                        bottom_tab.set(BottomTab::PeHeaders);
                    }
    ...
    let on_xor_brute = {
        ...
        Callback::from(move |_: MouseEvent| {
            ...
            spawn_local(async move {
                ...
                match invoke::<_, Vec<XorCandidate>>(
                    "xor_brute_force",
                    &json!({"sample_size":4096}),
                )
                .await
                {
                    Ok(c) => {
                        xor_candidates.set(c);
                        status.set("XOR brute-force done.".into());
                    }
    ...
    let on_xor_decode = {
        let key = xor_key_hex.clone();
        let result = xor_result_hex.clone();
        let selected = selected_byte.clone();
        let hex_offset = hex_offset.clone();
        let status = status.clone();
        Callback::from(move |_: MouseEvent| {
            ...
            spawn_local(async move {
                match invoke::<_, String>(
                    "xor_decode_region",
                    &json!({"offset":offset,"length":256,"key_hex":key}),
                )
                .await
                {
                    Ok(h) => result.set(h),
    ...
    let str_filtered: Vec<_> = if str_filter.is_empty() {
        ...
        extracted_strs
            .iter()
            .filter(|s| s.value.to_lowercase().contains(&f))
            .collect()
    };
    ...
    <header class="topbar">
        ...
        <div class="topbar-actions">
            <button class="btn-primary"  onclick={on_open_file.clone()}>{"⬆ Open"}</button>
            <button class="btn-accent"   onclick={on_yara_scan.clone()}>{"⚡ YARA"}</button>
            <button class="btn-ghost"    onclick={on_disasm.clone()}>{"⚙ Disasm"}</button>
            <button class="btn-ghost"    onclick={on_entropy.clone()}>{"📊 Entropy"}</button>
            <button class="btn-ghost"    onclick={on_extract_strings.clone()}>{"🔤 Strings"}</button>
            <button class="btn-ghost"    onclick={on_parse_headers.clone()}>{"🗂 Headers"}</button>
        </div>
        ...
        <div class="topbar-right">
            ...
            <select class="sel" onchange={{
                ...
                Callback::from(move |e: Event| {
                    if let Some(s) = e.target_dyn_into::<web_sys::HtmlSelectElement>() {
                        arch.set(s.value());
                    }
                })
            }}>
                { for ["x86_64","x86_32","arm64","arm"].iter().map(|a| html!{
                    <option value={*a} selected={disasm_arch.as_str() == *a}>{a}</option>
                })}
            </select>
        </div>
    </header>
        ...
        <section class="panel hex-panel">
            <div class="panel-title">
                ...
                <div class="hex-nav">
                    ...
                    />
                    <button class="btn-xs" onclick={on_jump.clone()}>{"Go"}</button>
                    <button class="btn-xs" onclick={on_prev_page.clone()}>{"◀"}</button>
                    <button class="btn-xs" onclick={on_next_page.clone()}>{"▶"}</button>
                ...
                if disasm_rows.is_empty() {
                    ...
                    <table class="disasm-table">
                        ...
                        { for disasm_rows.iter().map(|row| {
                            let cls = if row.mnemonic.starts_with("call") {
                                "disasm-row call"
                            } else if row.mnemonic.starts_with('j') {
                                "disasm-row jmp"
                            } else if row.mnemonic == "ret" || row.mnemonic == "retn" {
                                "disasm-row ret"
                            } else {
                                "disasm-row"
                            };
    ...
    <footer class="bottom-area">
        ...
        { match *bottom_tab {
            ...
            BottomTab::YaraRules => html! {
                <div class="yara-editor-wrap">
                    ...
                    <div class="editor-actions">
                        <button class="btn-accent" onclick={on_yara_scan.clone()}>{"⚡ Run YARA Scan"}</button>
                    </div>
                </div>
            },
            ...
            BottomTab::YaraResults => html! {
                ...
                if yara_hits.is_empty() {
                    ...
                    <table class="hit-table">
                        ...
                        <tbody>
                        { for yara_hits.iter().map(|hit| html! {
                            <tr class="hit-row">
            ...
            BottomTab::XorDecoder => html! {
                <div class="decoder-wrap">
                    <div class="decoder-controls">
                        ...
                        />
                        <button class="btn-primary" onclick={on_xor_decode.clone()}>{"Decode 256B at cursor"}</button>
                        <button class="btn-ghost" onclick={on_xor_brute.clone()}>{"Brute-force 1-byte key"}</button>
                    </div>
                        ...
                        if !xor_candidates.is_empty() {
                            <div class="candidates">
                                ...
                                <table class="cand-table">
                                    ...
                                    <tbody>
                                    { for xor_candidates.iter().take(10).map(|c| html! {
                                        <tr>
            ...
            BottomTab::Base64 => html! {
                <div class="decoder-wrap">
                    <div class="decoder-controls">
                        <button class="btn-primary" onclick={on_b64_encode.clone()}>{"Encode 256B at cursor → Base64"}</button>
                        <button class="btn-ghost" onclick={on_b64_decode.clone()}>{"Decode input → Hex"}</button>
                    </div>
            ...
            BottomTab::Strings => html! {
                <div class="decoder-wrap">
                    <div class="decoder-controls">
                        ...
                        />
                        <button class="btn-primary" onclick={on_extract_strings.clone()}>{"🔤 Extract Strings"}</button>
                        <label class="meta-label" style="margin-left:12px;">{"Filter:"}</label>
                        <input
                            class="inp-jump"
                            type="text"
                            value={(*str_filter).clone()}
                    ...
                    if extracted_strs.is_empty() {
                        ...
                        <table class="hit-table">
                            ...
                            <tbody>
                            { for str_filtered.iter().take(5000).map(|s| html! {
                                <tr>
            ...
            BottomTab::Entropy => html! {
                <div class="decoder-wrap">
                    <div class="decoder-controls">
                        <button class="btn-primary" onclick={on_entropy.clone()}>{"📊 Compute Entropy (256B blocks)"}</button>
                        if let Some(s) = entropy_sum.as_ref() {
                            <span class="meta-label">{
                                format!(
                                    "  min {:.2}  max {:.2}  mean {:.2}  ({} blocks)",
                    ...
                    if let Some(sum) = entropy_sum.as_ref() {
                        <div class="entropy-wrap">
                            ...
                            <div class="entropy-legend">
                                { for [
                                    ("#1e2a38","Zeroed <1.0"),
                                    ("#166534","Plaintext 1–4.5"),
                                    ("#4fc3f7","Code 4.5–6.5"),
                                    ("#fb923c","Compressed 6.5–7.2"),
                            ...
                            <div class="entropy-table-wrap">
                                ...
                                <table class="hit-table">
                                    ...
                                    <tbody>
                                    { for sum.blocks.iter().filter(|b| b.entropy > 7.0).map(|b| html! {
                                        <tr>
            ...
            BottomTab::PeHeaders => html! {
                <div class="decoder-wrap">
                    <div class="decoder-controls">
                        <button class="btn-primary" onclick={on_parse_headers.clone()}>{"🗂 Parse Headers"}</button>
                        if let Some(h) = parsed_headers.as_ref() {
                            <span class="meta-label" style="margin-left:8px;">{
                                format!(
                                    "Format: {}  |  {} sections  |  {} imports  |  {} exports",
                            ...
                            <div class="sub-tabs" style="margin-left:auto;">
                                { for [("fields","Fields"),("sections","Sections"),("imports","Imports"),("exports","Exports")]
                                    .iter()
                                    .map(|(id, lbl)| {
                                        let pts = pe_sub_tab.clone();
                                        let id_str = id.to_string();
                                        let active = pe_sub_tab.as_str() == id_str.as_str();
                        ...
                        { match pe_sub_tab.as_str() {
                            ...
                            "imports" => html! {
                                <table class="hit-table">
                                    ...
                                    <tbody>
                                    { for headers.imports.iter().map(|i| html! {
                                        <tr>
                            ...
                            "exports" => html! {
                                <table class="hit-table">
                                    ...
                                    <tbody>
                                    { for headers.exports.iter().map(|e| html! {
                                        <tr>
                            ...
                            _ => html! {
                                ...
                                <table class="hit-table">
                                    ...
                                    <tbody>
                                    { for headers.fields.iter().map(|f| html! {
                                        <tr>
    ...
    html! {
        <div class="hex-row">
            ...
            { for row.bytes.iter().enumerate().map(|(i, &byte)| {
                ...
                let is_sel = sel.as_ref().copied() == Some(boff);
