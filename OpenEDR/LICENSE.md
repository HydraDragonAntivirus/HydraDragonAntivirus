...
HydraDragon fork note:

This fork replaces the proprietary NetFilter SDK dependency with the
HydraDragonFirewall named-pipe telemetry path and relies on the open-source
Detours/user-mode hook path in `edrav2` instead of MadCodeHook-specific build
assumptions.
