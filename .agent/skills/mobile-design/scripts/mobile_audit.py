...
class MobileAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        if is_react_native:
            ...
            has_ios_text_style = has_large_title or has_title_1 or has_headline or has_body
            ...
            if has_ios_text_style and len(font_sizes) > 3 and matching_ios < len(font_sizes) / 2:
        ...
        if is_react_native:
            ...
            has_material_text_style = has_display or has_headline_material or has_title_material or has_body_material or has_label

            # Check if using sp (scale-independent pixels)
            uses_sp = bool(re.search(r"\d+\s*sp\b", content))
            if has_material_text_style:
                if not uses_sp:
        ...
        if is_react_native:
            ...
            if has_hardcoded_gray and not (has_label or has_secondaryLabel or has_systemBackground):
        ...
        if has_debugger and not has_performance:
            self.warnings.append(f"[Debugging] {filename}: Debug tooling detected without mobile performance profiling hooks.")
    ...
    def get_report(self):
        return {"files_checked": self.files_checked, "issues": self.issues, "warnings": self.warnings, "passed_checks": self.passed_count, "compliant": len(self.issues) == 0}
