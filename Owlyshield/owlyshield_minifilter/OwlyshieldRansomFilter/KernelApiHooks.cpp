...
NTSTATUS NTAPI HookedNtTerminateProcess(_In_opt_ HANDLE ProcessHandle, _In_ NTSTATUS ExitStatus)
{
...
NTSTATUS NTAPI HookedNtOpenProcess(_Out_ PHANDLE ProcessHandle, _In_ ACCESS_MASK DesiredAccess,
            ...
            (DesiredAccess & PROCESS_CREATE_THREAD))
        {
            ...
            if (IsProtectedProcess((ULONG)(ULONG_PTR)targetPid))
            {
                DbgPrint("!!! KernelHook: Process %lu requesting dangerous access (0x%X) to process %lu\n",
                         (ULONG)(ULONG_PTR)currentPid, DesiredAccess, (ULONG)(ULONG_PTR)targetPid);
...
NTSTATUS NTAPI HookedNtWriteVirtualMemory(_In_ HANDLE ProcessHandle, _In_ PVOID BaseAddress, _In_ PVOID Buffer,
        ...
        if (targetPid != currentPid)
        {
            if (IsProtectedProcess((ULONG)(ULONG_PTR)targetPid))
            {
                DbgPrint("!!! KernelHook: Process %lu writing %Iu bytes to process %lu at 0x%p\n",
                         (ULONG)(ULONG_PTR)currentPid, NumberOfBytesToWrite, (ULONG)(ULONG_PTR)targetPid, BaseAddress);

                WCHAR info[128];
                RtlStringCbPrintfW(info, sizeof(info), L"Memory write: %Iu bytes at 0x%p", NumberOfBytesToWrite,
                                   BaseAddress);
        ...
        if (targetPid != currentPid)
        {
            if (IsProtectedProcess((ULONG)(ULONG_PTR)targetPid))
            {
                DbgPrint("!!! KernelHook: Process %lu creating remote thread in process %lu at 0x%p\n",
                         (ULONG)(ULONG_PTR)currentPid, (ULONG)(ULONG_PTR)targetPid, StartRoutine);
...
NTSTATUS NTAPI HookedNtSetInformationFile(_In_ HANDLE FileHandle, _Out_ PIO_STATUS_BLOCK IoStatusBlock,
                                          ...
                                          _In_ FILE_INFORMATION_CLASS FileInformationClass)
{
    ...
    return g_OriginalNtSetInformationFile(FileHandle, IoStatusBlock, FileInformation, Length, FileInformationClass);
}
...
NTSTATUS NTAPI HookedNtDeleteFile(_In_ POBJECT_ATTRIBUTES ObjectAttributes)
{
...
NTSTATUS NTAPI HookedNtAllocateVirtualMemory(_In_ HANDLE ProcessHandle, _Inout_ PVOID *BaseAddress,
                                             ...
                                             _In_ ULONG AllocationType, _In_ ULONG Protect)
{
        ...
        if (targetPid != currentPid && IsExecutableProtection(Protect))
        {
            if (IsProtectedProcess((ULONG)(ULONG_PTR)targetPid))
            {
                DbgPrint("!!! KernelHook: Process %lu allocating executable memory (0x%X) in process %lu\n",
                         (ULONG)(ULONG_PTR)currentPid, Protect, (ULONG)(ULONG_PTR)targetPid);
    ...
    return g_OriginalNtAllocateVirtualMemory(ProcessHandle, BaseAddress, ZeroBits, RegionSize, AllocationType, Protect);
}
...
NTSTATUS NTAPI HookedNtProtectVirtualMemory(_In_ HANDLE ProcessHandle, _Inout_ PVOID *BaseAddress,
                                            _Inout_ PSIZE_T RegionSize, _In_ ULONG NewProtect, _Out_ PULONG OldProtect)
{
    PEPROCESS targetProcess = NULL;
    HANDLE currentPid = PsGetCurrentProcessId();
    HANDLE targetPid = NULL;
        ...
        if (targetPid != currentPid && IsExecutableProtection(NewProtect))
        {
            if (IsProtectedProcess((ULONG)(ULONG_PTR)targetPid))
            {
                DbgPrint("!!! KernelHook: Process %lu changing memory protection to executable (0x%X) in process %lu\n",
                         (ULONG)(ULONG_PTR)currentPid, NewProtect, (ULONG)(ULONG_PTR)targetPid);

                WCHAR info[128];
                RtlStringCbPrintfW(info, sizeof(info), L"DEP bypass attempt: NewProtect=0x%X at 0x%p", NewProtect,
...
NTSTATUS NTAPI HookedNtQueueApcThread(_In_ HANDLE ThreadHandle, _In_ PVOID ApcRoutine, _In_opt_ PVOID ApcArgument1,
        ...
        if (targetPid != currentPid)
        {
            if (IsProtectedProcess((ULONG)(ULONG_PTR)targetPid))
            {
                DbgPrint("!!! KernelHook: Process %lu queuing APC in process %lu thread (Routine: 0x%p)\n",
                         (ULONG)(ULONG_PTR)currentPid, (ULONG)(ULONG_PTR)targetPid, ApcRoutine);
...
NTSTATUS NTAPI HookedNtSetContextThread(_In_ HANDLE ThreadHandle, _In_ PCONTEXT ThreadContext)
{
    PETHREAD targetThread = NULL;
    PEPROCESS targetProcess = NULL;
    HANDLE currentPid = PsGetCurrentProcessId();
...
#ifdef _AMD64_
                WCHAR info[128];
                RtlStringCbPrintfW(info, sizeof(info), L"Thread hijacking: RIP=0x%llX",
                                   ThreadContext ? ThreadContext->Rip : 0);
#else
                WCHAR info[128];
                RtlStringCbPrintfW(info, sizeof(info), L"Thread hijacking: EIP=0x%X",
...
NTSTATUS NTAPI HookedNtLoadDriver(_In_ PUNICODE_STRING DriverServiceName)
{
    HANDLE currentPid = PsGetCurrentProcessId();
    ...
    {
        RtlStringCbPrintfW(info, sizeof(info), L"Driver load: %wZ", DriverServiceName);
    }
    else
    {
...
NTSTATUS NTAPI HookedZwCreateSection(_Out_ PHANDLE SectionHandle, _In_ ACCESS_MASK DesiredAccess,
                                     ...
                                     _In_opt_ HANDLE FileHandle)
{
    ...
    if (IsExecutableProtection(SectionPageProtection))
    {
        DbgPrint("!!! KernelHook: Process %lu creating executable section (Protection: 0x%X)\n",
                 (ULONG)(ULONG_PTR)currentPid, SectionPageProtection);
...
NTSTATUS NTAPI HookedZwMapViewOfSection(_In_ HANDLE SectionHandle, _In_ HANDLE ProcessHandle,
                                        ...
                                        _In_ ULONG Win32Protect)
{
        ...
        if (targetPid != currentPid && IsExecutableProtection(Win32Protect))
        {
            if (IsProtectedProcess((ULONG)(ULONG_PTR)targetPid))
            {
                DbgPrint("!!! KernelHook: Process %lu mapping executable section into process %lu (Protection: 0x%X)\n",
                         (ULONG)(ULONG_PTR)currentPid, (ULONG)(ULONG_PTR)targetPid, Win32Protect);
    ...
    return g_OriginalZwMapViewOfSection(SectionHandle, ProcessHandle, BaseAddress, ZeroBits, CommitSize, SectionOffset,
...
NTSTATUS InstallKernelApiHooks(VOID)
{
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtTerminateProcess, "NtTerminateProcess",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtOpenProcess, "NtOpenProcess",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtWriteVirtualMemory, "NtWriteVirtualMemory",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtCreateThreadEx, "NtCreateThreadEx",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtAllocateVirtualMemory, "NtAllocateVirtualMemory",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtProtectVirtualMemory, "NtProtectVirtualMemory",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtQueueApcThread, "NtQueueApcThread",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtSetContextThread, "NtSetContextThread",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedNtLoadDriver, "NtLoadDriver",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedZwCreateSection, "ZwCreateSection",
    ...
    if (targetFunc != NULL)
    {
        status = HookEngineInstallHook(targetFunc, (PVOID)HookedZwMapViewOfSection, "ZwMapViewOfSection",
