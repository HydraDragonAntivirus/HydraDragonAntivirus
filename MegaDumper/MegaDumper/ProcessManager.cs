...
namespace Mega_Dumper
{
    ...
    {
        internal static readonly OrdinalCaseInsensitiveComparer Default = new();
    ...
    public partial class ProcessManager : Form
    {
        ...
        public static byte[] ToByteArray(StringDictionary sd, bool unicode)
        {
            ...
            byte[] envBlock = unicode ? Encoding.Unicode.GetBytes(stringBuff.ToString()) : Encoding.Default.GetBytes(stringBuff.ToString());
        ...
        private static void LoopWhileModulesAreLoadedNt(uint procid, IntPtr htread)
        {
            IntPtr processHandle = IntPtr.Zero;
            try
            {
                processHandle = OpenProcess(ProcessAccess.QueryInformation | ProcessAccess.VMRead, false, procid);
            }
            catch
            {
            }
        ...
        private void Button2Click(object sender, EventArgs e)
        {
            ...
            if (filename != "" && File.Exists(filename))
            {
                ...
                if (enviroments.Length != 0)
                {
                    if (enviroments.Length >= 3 && enviroments.Contains("="))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                processflags |= (int)ProcessCreationFlags.CREATE_UNICODE_ENVIRONMENT;
                                unicode = true;
                            }
                            StringDictionary environmentVariables = new();
                ...
                STARTUPINFO structure = new();
                PROCESS_INFORMATION process_information = new();
                ...
                if (checkBox3.Checked)
                {
                    ...
                    for (int k = 0; k < 50; k++)  // try it 50 times!
                    {
                        ...
                        if (modules?.Length > 0)
                        {
                            for (int i = 0; i < modules.Length; i++)
                            {
                                if (modules[i].baseName.Length > 10 && modules[i].baseName.StartsWith("msvcr", StringComparison.OrdinalIgnoreCase))
                                {
        ...
        */
        ...
        public void HookCreateProcess(IntPtr kernelbase)
        {
            ...
            uint BytesRead = 0;
            if (!ReadProcessMemory(hprocess, (IntPtr)((long)kernelbase + CreateProcessWRva), Forread, 0x500, ref BytesRead))
                return;
            ...
            long CreateProcessWcall = (long)kernelbase + CreateProcessWRva + count;
            int tobecalled1 = BitConverter.ToInt32(Forread, count + 1);
            tobecalled1 = tobecalled1 + (int)CreateProcessWcall + 5;
            IntPtr CreateProcessWPatch1 = (IntPtr)(CreateProcessWcall + 1);

            if (!ReadProcessMemory(hprocess, (IntPtr)((long)kernelbase + CreateProcessARva), Forread, 0x500, ref BytesRead))
                return;
            ...
            long CreateProcessAcall = (long)kernelbase + CreateProcessARva + count;
            int tobecalled2 = BitConverter.ToInt32(Forread, count + 1);
            tobecalled2 = tobecalled2 + (int)CreateProcessAcall + 5;
            IntPtr CreateProcessAPatch2 = (IntPtr)(CreateProcessAcall + 1);
        ...
        public void LogmemcpyInit(IntPtr MSVCR80base)
        {
            ...
            uint BytesRead = 0;
            if (!ReadProcessMemory(hprocess, (IntPtr)((long)MSVCR80base + memcpyRva), Forread, 0x50, ref BytesRead))
                return;
            ...
            WriteProcessMemory(hprocess, (IntPtr)((long)MSVCR80base + memcpyRva + count), oldbytes, (uint)oldbytes.Length, out BytesRead);
        ...
        private void ProcessManagerFormClosing(object sender, FormClosingEventArgs e)
        {
            GetExitCodeProcess(hprocess, out uint exitcode);
