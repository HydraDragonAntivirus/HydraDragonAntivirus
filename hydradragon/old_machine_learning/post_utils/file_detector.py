...
def hash_file(filepath):
    ...
    try:
        ...
        return {"filepath": filepath, "hash": file_hash, "size": os.path.getsize(filepath)}
    except Exception as e:
        return {"filepath": filepath, "hash": None, "error": str(e)}
...
def check_malicious_files():
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        with tqdm(total=len(malicious_files), desc="Hashing malicious dataset", unit="files", smoothing=0.1) as pbar:
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        with tqdm(total=len(data2_files), desc="Hashing data2", unit="files", smoothing=0.1) as pbar:
            for future in as_completed(futures):
                results = future.result()
