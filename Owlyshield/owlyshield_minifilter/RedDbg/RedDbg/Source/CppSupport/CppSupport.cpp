...
inline ULONG64 GetPoolFlagsFromType(POOL_TYPE PoolType) {
    if (PoolType == NonPagedPool || PoolType == NonPagedPoolExecute || PoolType == NonPagedPoolNx)
...
void *__cdecl operator new(size_t Size)
{
    ...
    // ExAllocatePool2 zero-initializes memory by default, but we'll leave this to match original semantics
    if (Pointer)
...
{
    void *Pointer = ExAllocatePool2(GetPoolFlagsFromType(PoolType), Size, CrtPoolTag);
    if (Pointer)
        RtlZeroMemory(Pointer, Size);
    return Pointer;
}
...
[[noreturn]] static void RaiseException(ULONG BugCheckCode)
{
    KdBreakPoint();
...
[[noreturn]] void __cdecl RaiseHandler(const stdext::exception &)
{
    RaiseException(KMODE_EXCEPTION_NOT_HANDLED);
}
...
[[noreturn]] void __cdecl _invoke_watson(wchar_t const *const expression, wchar_t const *const function_name,
