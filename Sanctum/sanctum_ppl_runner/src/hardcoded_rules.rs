...
fn section_len(name: &str, text: &str) -> Result<u32, String> {
    ...
    if bytes > MAX_RULE_BLOB_SECTION_SIZE {
        return Err(format!(
            "{name} embedded rules section is too large: {bytes} bytes > {MAX_RULE_BLOB_SECTION_SIZE} bytes"
        ));
    }
    u32::try_from(bytes).map_err(|_| format!("{name} embedded rule section length overflow"))
}
