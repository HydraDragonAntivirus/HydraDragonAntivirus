...
def enable_debug_privilege():
    ...
    try:
        ...
        advapi32.OpenProcessToken(k32.GetCurrentProcess(), 0x0020 | 0x0008, ctypes.byref(h_token))
        luid = LUID()
...
def _create_remote_thread(h_proc, start_addr, param, pid: int, label: str):
    ...
    h_thread = k32.CreateRemoteThread(h_proc, None, 0, start_addr, param, 0, ctypes.byref(thread_id))
    if h_thread:
    ...
    status = ntdll.NtCreateThreadEx(ctypes.byref(h_nt), 0x1FFFFF, None, h_proc, start_addr, param, 0, 0, 0, 0, None)
...
def _wait_thread_exit(h_thread, timeout_ms: int, label: str):
    ...
    if result == WAIT_OBJECT_0:
        ...
        logger.warning(f"[Hook] {label}: GetExitCodeThread failed ({ctypes.get_last_error()})")
        return True, None
...
def _find_remote_module_base(pid: int, module_name: str, retries: int = 20, delay: float = 0.1):
    ...
    for _ in range(retries):
        ...
        if snap and snap != INVALID_HANDLE_VALUE:
            try:
                ...
                while ok:
                    if entry.szModule.lower() == target or os.path.basename(entry.szExePath).lower() == target:
...
def inject(pid: int, name: str) -> bool:
    ...
    try:
        ...
        logger.info(f"[Hook] Injecting into {name} (PID: {pid}, {'x64' if is_64 else 'x86'})")
        ...
        if not h_proc:
            logger.error(f"[Hook] OpenProcess failed PID {pid} (error {ctypes.get_last_error()})")
            return False
        ...
        try:
            ...
            if not mem:
                logger.error(f"[Hook] VirtualAllocEx failed PID {pid} (error {ctypes.get_last_error()})")
                return False

            written = ctypes.c_size_t(0)
            if not k32.WriteProcessMemory(h_proc, mem, path_bytes, len(path_bytes), ctypes.byref(written)):
                logger.error(f"[Hook] WriteProcessMemory failed PID {pid} (error {ctypes.get_last_error()})")
                return False
            ...
            try:
                ...
                h_start = _create_remote_thread(h_proc, remote_fn, None, pid, "HydraStartHook")
                if not h_start:
                    return False
                try:
                    start_ok, start_exit = _wait_thread_exit(h_start, 5000, "HydraStartHook")
                finally:
                ...
                if start_exit and start_exit != 0:
                    logger.warning(f"[Hook] HydraStartHook returned error {start_exit} for {name} PID {pid}")
                else:
...
def _find_pids_for_exe(exe_path: str) -> list[tuple[int, str]]:
    ...
    for proc in psutil.process_iter(["pid", "name", "exe"]):
        try:
            ...
            if not proc_exe:
                continue
            if os.path.normcase(os.path.normpath(proc_exe)).startswith(_EXCLUDED_DIR_NORM):
                continue
            if os.path.normcase(os.path.normpath(proc_exe)) == target:
                results.append((proc.info["pid"], proc.info["name"] or f"pid_{proc.pid}"))
