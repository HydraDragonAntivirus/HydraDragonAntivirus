...
def scan_directory(root_dir, max_size_mb=10, existing_hashes=None):
    """
    Recursively scan the directory for unique JS files under a size threshold,
    ...
    safe_print(f"\nScanning '{root_dir}' for JS files <= {max_size_mb}MB (ignoring access errors)...")
    start = time.time()
    ...
    for dirpath, dirs, files in os.walk(root_dir, onerror=lambda e: None):
        for fname in files:
            ...
            if total_scanned % 1000 == 0:
                safe_print(f"[Progress] Scanned {total_scanned} files, found {len(found)} new JS files so far...")
...
def main():
    ...
    safe_print("=" * 60)
    safe_print("JS File Scanner - Multi-Mode Operation")
    safe_print("=" * 60)
    safe_print("\nSelect Mode:")
