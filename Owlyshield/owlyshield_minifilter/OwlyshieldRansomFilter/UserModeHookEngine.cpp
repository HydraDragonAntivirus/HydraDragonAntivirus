...
UCHAR g_ShellcodeTemplate[] = {
    ...
    0x0F, 0x85, 0x7F, 0x01, 0x00, 0x00,                         // jnz SKIP (-> offset 403)
    ...
    // ===== FILL HOOK_EVENT_DATA (Starts at RSP+0x60) =====
    // EventType PATCHED at offset 63
    0xC7, 0x44, 0x24, 0x60, 0x11, 0x11, 0x11, 0x11,             // mov dword [rsp+60h], 11111111h
    // ProcessId PATCHED at offset 71
    0xC7, 0x44, 0x24, 0x64, 0x22, 0x22, 0x22, 0x22,             // mov dword [rsp+64h], 22222222h
    
    // FunctionName 8x qwords 
    0x48, 0xB8, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, // mov rax, 66... (PATCH at 77)
    0x48, 0x89, 0x44, 0x24, 0x68,                               // mov [rsp+68h], rax
    0x48, 0xB8, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77, // mov rax, 77... (PATCH at 92)
    0x48, 0x89, 0x44, 0x24, 0x70,                               // mov [rsp+70h], rax
    0x48, 0xB8, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, 0x88, // mov rax, 88... (PATCH at 107)
    0x48, 0x89, 0x44, 0x24, 0x78,                               // mov [rsp+78h], rax
    0x48, 0xB8, 0x99, 0x99, 0x99, 0x99, 0x99, 0x99, 0x99, 0x99, // mov rax, 99... (PATCH at 122)
    0x48, 0x89, 0x84, 0x24, 0x80, 0x00, 0x00, 0x00,             // mov [rsp+80h], rax
    0x48, 0xB8, 0xAB, 0xAB, 0xAB, 0xAB, 0xAB, 0xAB, 0xAB, 0xAB, // mov rax, AB... (PATCH at 140)
    0x48, 0x89, 0x84, 0x24, 0x88, 0x00, 0x00, 0x00,             // mov [rsp+88h], rax
    0x48, 0xB8, 0xBC, 0xBC, 0xBC, 0xBC, 0xBC, 0xBC, 0xBC, 0xBC, // mov rax, BC... (PATCH at 158)
    0x48, 0x89, 0x84, 0x24, 0x90, 0x00, 0x00, 0x00,             // mov [rsp+90h], rax
    0x48, 0xB8, 0xCD, 0xCD, 0xCD, 0xCD, 0xCD, 0xCD, 0xCD, 0xCD, // mov rax, CD... (PATCH at 176)
    0x48, 0x89, 0x84, 0x24, 0x98, 0x00, 0x00, 0x00,             // mov [rsp+98h], rax
    0x48, 0xB8, 0xDE, 0xDE, 0xDE, 0xDE, 0xDE, 0xDE, 0xDE, 0xDE, // mov rax, DE... (PATCH at 194)
    0x48, 0x89, 0x84, 0x24, 0xA0, 0x00, 0x00, 0x00,             // mov [rsp+A0h], rax

    // Arg1 = Original RCX (Fetch from rsp+0x128 -> Store in rsp+0xA8)
    0x48, 0x8B, 0x84, 0x24, 0x28, 0x01, 0x00, 0x00,             // mov rax, [rsp+128h]
    0x48, 0x89, 0x84, 0x24, 0xA8, 0x00, 0x00, 0x00,             // mov [rsp+A8h], rax

    // Arg2 = Original RDX (Fetch from rsp+0x120 -> Store in rsp+0xB0)
    0x48, 0x8B, 0x84, 0x24, 0x20, 0x01, 0x00, 0x00,             // mov rax, [rsp+120h]
    0x48, 0x89, 0x84, 0x24, 0xB0, 0x00, 0x00, 0x00,             // mov [rsp+B0h], rax

    // Zero IO_STATUS_BLOCK (rsp+0x50, 16 bytes)
    0x48, 0x31, 0xC0,                                           // xor rax, rax
    0x48, 0x89, 0x44, 0x24, 0x50,                               // mov [rsp+50h], rax
    0x48, 0x89, 0x44, 0x24, 0x58,                               // mov [rsp+58h], rax

    // ===== PREPARE NtDeviceIoControlFile =====
    // FileHandle (RCX) PATCHED at offset 257
    0x48, 0xB9, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 
    0x48, 0x31, 0xD2,                                           // xor rdx, rdx (Arg2=Event)
    0x4D, 0x31, 0xC0,                                           // xor r8, r8   (Arg3=ApcRoutine)
    0x4D, 0x31, 0xC9,                                           // xor r9, r9   (Arg4=ApcContext)

    // IoStatusBlock (rsp+0x50 -> Arg5 at rsp+0x20)
    0x48, 0x8D, 0x44, 0x24, 0x50,                               // lea rax, [rsp+50h]
    0x48, 0x89, 0x44, 0x24, 0x20,                               // mov [rsp+20h], rax

    // IoControlCode (Arg6 at rsp+0x28) PATCHED at offset 288
    0xC7, 0x44, 0x24, 0x28, 0xAA, 0xAA, 0xAA, 0xAA,             // mov dword [rsp+28h], AA...

    // InputBuffer (rsp+0x60 -> Arg7 at rsp+0x30)
    0x48, 0x8D, 0x44, 0x24, 0x60,                               // lea rax, [rsp+60h]
    0x48, 0x89, 0x44, 0x24, 0x30,                               // mov [rsp+30h], rax

    // InputBufferLength = 88 bytes (Arg8 at rsp+0x38)
    0xC7, 0x44, 0x24, 0x38, 0x58, 0x00, 0x00, 0x00,             // mov dword [rsp+38h], 58h

    // OutputBuffer = NULL (Arg9 at rsp+0x40)
    0x48, 0xC7, 0x44, 0x24, 0x40, 0x00, 0x00, 0x00, 0x00,       // mov qword [rsp+40h], 0

    // OutputBufferLength = 0 (Arg10 at rsp+0x48)
    0xC7, 0x44, 0x24, 0x48, 0x00, 0x00, 0x00, 0x00,             // mov dword [rsp+48h], 0

    // NtDeviceIoControlFile address PATCHED at offset 329
    0x48, 0xB8, 0x44, 0x44, 0x44, 0x44, 0x44, 0x44, 0x44, 0x44, // mov rax, 44...
    0xFF, 0xD0,                                                 // call rax

    // ===== CLEAR BUSY FLAG (FLAG_ADDR patched at offset 342) =====
    0x50,                                                       // push rax
    0x48, 0xB8, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, // mov rax, FLAG_ADDR
    ...
    // ===== STOLEN BYTES (14 NOPs) =====
    0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90,

    // ===== JUMP TO GATEWAY (0x5555 PATCHED at offset 393) =====
    0x48, 0xB8, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, // mov rax, 55...
    0xFF, 0xE0,                                                 // jmp rax

    // ===== SKIP TARGET: busy path (0xBBBB PATCHED at offset 406) =====
    0x58,                                                       // pop rax
    0x48, 0xB8, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, // mov rax, BB...
    0xFF, 0xE0,                                                 // jmp rax
                          ...
                          _In_ PVOID TargetNtDeviceIo, _In_ ULONG IoControlCode)
{
    ...
    // Safety checks (Updated for new shellcode lengths)
    if (*(PULONG)(shellcode + 63) != 0x11111111 || *(PULONG)(shellcode + 71) != 0x22222222 ||
        *(PULONGLONG)(shellcode + 257) != 0x3333333333333333ULL || *(PULONG)(shellcode + 288) != 0xAAAAAAAA ||
        *(PULONGLONG)(shellcode + 329) != 0x4444444444444444ULL ||
        *(PULONGLONG)(shellcode + 393) != 0x5555555555555555ULL)
    {
        KeUnstackDetachProcess(&apcState);
        return STATUS_INVALID_IMAGE_FORMAT;
    }

    // Patch FLAG_ADDR (3 locations)
    *(PVOID *)(shellcode + 3) = flagAddr;
    *(PVOID *)(shellcode + 24) = flagAddr;
    *(PVOID *)(shellcode + 342) = flagAddr;

    // Patch EventType and ProcessId
    *(PULONG)(shellcode + 63) = EventId;
    *(PULONG)(shellcode + 71) = ProcessId;

    // Patch FunctionName (Updated Offsets)
    ...
    *(PHANDLE)(shellcode + 257) = HookEntry->DriverDeviceHandle;
    *(PULONG)(shellcode + 288) = IoControlCode;
    *(PVOID *)(shellcode + 329) = TargetNtDeviceIo;

    // NOP sled (gateway handles real execution)
    RtlFillMemory(shellcode + 377, 14, 0x90);

    // Patch gateway address for BOTH normal and busy (skip) paths
    *(PVOID *)(shellcode + 393) = gatewayAddress;
    *(PVOID *)(shellcode + 406) = gatewayAddress;
