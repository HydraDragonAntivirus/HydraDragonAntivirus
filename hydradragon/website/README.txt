Active whitelists must be come from non dead repos
Current whitelist repos:
https://github.com/T145/black-mirror (with CIDR)
https://github.com/amieiro/disposable-email-domains
https://github.com/bitwire-it/ipblocklist
https://github.com/Levi2288/AdvancedBlockList
Current obsolete but still used repos:
https://github.com/bitwire-it/ipblocklist
Current phishing active-inactive repos:
https://github.com/Phishing-Database/Phishing.Database?tab=readme-ov-file
Current heuristic malware or phishing ipv4-ipv6 repo:
https://github.com/HydraDragonAntivirus/HydraDragonAntivirusSearchEngine
IPv4BruteForce.txt comes from: 
https://list.rtbh.com.tr/
listed_email_365.txt comes from:
https://www.stopforumspam.com/downloads
Current popularity reference: 
tranco-list.eu/list/PL9GJ/1000000
TODO: Maybe look at this even if many false positives here:
https://github.com/juergen2025sys/NETSHIELD


