...
fn extract_ascii(bytes: &[u8], min_len: usize, out: &mut Vec<StringHit>) {
    ...
    if let Some(s) = start {
        if bytes.len() - s >= min_len {
            out.push(StringHit {
                value: String::from_utf8_lossy(&bytes[s..]).to_string(),
                offset: s,
                encoding: "ascii".into(),
            });
        }
...
fn maybe_push_reverse(raw: &str, out: &mut Vec<DecodedString>, seen: &mut HashSet<String>) {
    ...
    if URL_RE.is_match(&reversed)
        || POWERSHELL_HINT_RE.is_match(&reversed)
        || reversed.contains(".exe")
        || reversed.contains("HKEY_")
    {
...
fn maybe_push_rot13(raw: &str, out: &mut Vec<DecodedString>, seen: &mut HashSet<String>) {
    ...
    if URL_RE.is_match(&decoded)
        || POWERSHELL_HINT_RE.is_match(&decoded)
        || decoded.contains(".exe")
    {
