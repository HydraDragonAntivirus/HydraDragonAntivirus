...
def run_disassembler_binary(binary_path, file_name, out_file_name):
    # Ensure the binary exists
    if not os.path.isfile(binary_path):
        raise FileNotFoundError(f"The binary '{binary_path}' does not exist. You can specify a path to a similar disassembler version using the --path (-p) argument.")
    ...
    with open(out_file_name, "w") as outfile:
        ...
        try:
            ...
            if result.stderr:
                raise RuntimeError(f"Binary execution failed with status code {result.returncode}: {result.stderr.strip()}")
