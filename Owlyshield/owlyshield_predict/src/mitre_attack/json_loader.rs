...
pub fn load_all_techniques_from_json() -> HashMap<String, MitreTechnique> {
    ...
    match std::fs::read_to_string(mitre_path) {
        Ok(json_data) => {
            match serde_json::from_str::<MitreBundle>(&json_data) {
                Ok(bundle) => {
...
fn parse_mitre_bundle(bundle: MitreBundle) -> HashMap<String, MitreTechnique> {
    ...
    for object in bundle.objects {
        ...
        let technique_id = object.external_references
            .as_ref()
            .and_then(|refs| {
                refs.iter()
        ...
        if let (Some(id), Some(name)) = (technique_id, object.name) {
            ...
            let tactic = object.kill_chain_phases
                .as_ref()
            ...
            let technique = MitreTechnique {
                ...
                name,
                tactic,
                description: object.description.unwrap_or_else(|| "No description available".to_string()),
                severity,
            };
...
mod tests {
    ...
    fn test_load_techniques() {
        ...
        assert!(techniques.contains_key("T1486"), "Should contain ransomware technique");
        assert!(techniques.contains_key("T1055"), "Should contain process injection");
        assert!(techniques.contains_key("T1003"), "Should contain credential dumping");
    }
