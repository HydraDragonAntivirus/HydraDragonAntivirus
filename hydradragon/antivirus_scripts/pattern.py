...
cdn_attachment_pattern_standard = re.compile(r"https://(?:cdn\.discordapp\.com|media\.discordapp\.net)/attachments/\d+/\d+/[A-Za-z0-9_\-\.%]+(?:\?size=\d+)?")
...
ZIP_JOIN = re.compile(r"""(?:""?|''?)(?:\w*\.)?join\(\s*\(chr\(\(x\s*-\s*y\)\s*%\s*128\)\s*for\s*x\s*,\s*y\s*in\s*zip\(\s*(\[[^\]]*\])\s*,\s*(\[[^\]]*\])\s*\)\)\)""", re.DOTALL)

# Pattern for chained .join calls: literal.join(...).join(...)
CHAINED_JOIN = re.compile(r"(\(['\"][^'\"]*['\"]\))\.(?:join\([^)]*\))+")
...
def build_ip_patterns():
    ...
    def _collect_from_encoded(text):
        ...
        for m in base64_candidate.finditer(text):
            ...
            for im in re.finditer(ipv4_inner, dec):
                results.append({"type": "IPv4", "value": im.group(0), "span": (m.start(1), m.end(1)), "source": "base64", "original_fragment": frag})
            for im in re.finditer(ipv6_inner, dec, re.IGNORECASE):
                results.append({"type": "IPv6", "value": im.group(0), "span": (m.start(1), m.end(1)), "source": "base64", "original_fragment": frag})
        ...
        for m in base32_candidate.finditer(text):
            ...
            if not dec:
                continue
            for im in re.finditer(ipv4_inner, dec):
                results.append({"type": "IPv4", "value": im.group(0), "span": (m.start(1), m.end(1)), "source": "base32", "original_fragment": frag})
            for im in re.finditer(ipv6_inner, dec, re.IGNORECASE):
                results.append({"type": "IPv6", "value": im.group(0), "span": (m.start(1), m.end(1)), "source": "base32", "original_fragment": frag})
    ...
    def _collect_from_reversed(text):
        ...
        for m in reversed_candidates:
            ...
            if ipv4_match:
                results.append({"type": "IPv4", "value": ipv4_match.group(0), "span": (m.start(1), m.end(1)), "source": "reversed", "original_fragment": frag})
            if ipv6_match:
                results.append({"type": "IPv6", "value": ipv6_match.group(0), "span": (m.start(1), m.end(1)), "source": "reversed", "original_fragment": frag})
        return results
