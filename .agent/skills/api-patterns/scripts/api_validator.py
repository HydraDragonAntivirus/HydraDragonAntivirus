...
def check_api_code(file_path: Path) -> dict:
    ...
    try:
        ...
        status_patterns = [r"status\s*\(\s*\d{3}\s*\)", r"statusCode\s*[=:]\s*\d{3}", r"HttpStatus\.", r"status_code\s*=\s*\d{3}", r"\.status\(\d{3}\)", r"res\.status\("]
        ...
        validation_patterns = [r"validate", r"schema", r"zod", r"joi", r"yup", r"pydantic", r"@Body\(", r"@Query\("]
        ...
        auth_patterns = [r"auth", r"jwt", r"bearer", r"token", r"middleware", r"guard", r"@Authenticated"]
