...
impl Pattern {
    ...
    /// ClamAV's `partno` parts. Computed on demand (gappy patterns are a minority).
    pub(crate) fn gap_parts(&self) -> Vec<(Pattern, Option<(usize, usize)>)> {
        ...
        parts.push((
            Pattern::from_parsed(part_inst, part_specials, false),
            gap_before,
        ));
        parts
    }
