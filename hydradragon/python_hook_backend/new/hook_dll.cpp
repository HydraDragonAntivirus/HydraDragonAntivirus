...
static HMODULE GetLoadedPythonRuntimeModule(char *outDllName,
                                            size_t outDllNameLen) {
  for (int i = 15; i >= 6; --i) {
    char dllName[32];
    _snprintf_s(dllName, sizeof(dllName), _TRUNCATE, "python3%d.dll", i);
    dllName[sizeof(dllName) - 1] = '\0';
    HMODULE hPy = GetModuleHandleA(dllName);
    if (hPy) {
      if (outDllName && outDllNameLen > 0) {
        strncpy_s(outDllName, outDllNameLen, dllName, _TRUNCATE);
        outDllName[outDllNameLen - 1] = '\0';
      }
      return hPy;
    }
  }

  HMODULE hPy = GetModuleHandleA("python3.dll");
  if (hPy && outDllName && outDllNameLen > 0) {
    strncpy_s(outDllName, outDllNameLen, "python3.dll", _TRUNCATE);
    outDllName[outDllNameLen - 1] = '\0';
  }
  return hPy;
}
...
// 3. The Hook Handler (Fixes C2440)
void* __fastcall UnmarshalHook(void* tstate, void* mod_consts, __int64 data_stream) {
    RemoveTrampoline();

    typedef void* (__fastcall* tOriginal)(void*, void*, __int64);
    void* resultObject = ((tOriginal)g_targetAddr)(tstate, mod_consts, data_stream);

    if (resultObject && pPyUnicode_AsUTF8) {
        // FIXED C2440: Explicitly cast void* to const char*
        const char* decoded = (const char*)pPyUnicode_AsUTF8(resultObject);
        if (decoded && strlen(decoded) > 0) {
            // Logs to your existing g_logFile system
            dbgPrintf("[BLOB-DATA] %s", decoded);
        }
    }

    PlaceTrampoline(g_targetAddr, (void*)UnmarshalHook);
    return resultObject;
}

// 4. Initialization Entry Point
void StartBlobCapture() {
    void* fnAddr = FindNuitkaUnmarshal("app_loop.dll");
    if (!fnAddr)
        return;

    char dllName[32] = {0};
    HMODULE hPy = GetLoadedPythonRuntimeModule(dllName, sizeof(dllName));
    if (!hPy) {
        dbgPrintf("[SYSTEM] Blob capture skipped: no loaded Python runtime module found.\n");
        return;
    }

    pPyUnicode_AsUTF8 =
        (tPyUnicode_AsUTF8)GetProcAddress(hPy, "PyUnicode_AsUTF8");
    if (!pPyUnicode_AsUTF8) {
        dbgPrintf("[SYSTEM] Blob capture skipped: PyUnicode_AsUTF8 was not exported by %s.\n",
                  dllName[0] ? dllName : "<unknown>");
        return;
    }

    PlaceTrampoline(fnAddr, (void*)UnmarshalHook);
    dbgPrintf("[SYSTEM] Generic Unmasking Hook Placed using %s.\n",
              dllName[0] ? dllName : "<unknown>");
}
