...
class JSFeatureExtractor:
    def __init__(self):
        ...
        self.crypto_patterns = [r"crypto", r"CryptoJS", r"aes", r"des", r"rsa", r"md5", r"sha1", r"sha256", r"sha512", r"encrypt", r"decrypt", r"cipher"]

        self.network_patterns = [r"http[s]?://", r"ws[s]?://", r"ftp://", r"fetch\s*\(", r"XMLHttpRequest", r"\.send\s*\(", r"\.open\s*\(", r"WebSocket", r"EventSource"]

        self.file_system_patterns = [r"FileSystemObject", r"readFile", r"writeFile", r"createTextFile", r"OpenTextFile", r"DeleteFile", r"CopyFile", r"MoveFile"]

        self.registry_patterns = [r"RegRead", r"RegWrite", r"RegDelete", r"HKEY_", r"HKLM", r"HKCU", r"HKCR"]

        self.process_patterns = [r"Run\s*\(", r"Exec\s*\(", r"ShellExecute", r"CreateObject\s*\(", r"GetObject\s*\(", r"\.Run\s*\(", r"\.Exec\s*\("]
    ...
    def analyze_obfuscation(self, code: str) -> Dict[str, Any]:
        ...
        obfuscation = {"hex_encoded_strings": 0, "unicode_encoded_strings": 0, "char_code_usage": 0, "base64_usage": 0, "escape_usage": 0, "bracket_notation_calls": 0, "total_obfuscation_score": 0, "is_likely_obfuscated": False}
    ...
    def analyze_suspicious_patterns(self, code: str) -> Dict[str, Any]:
        ...
        patterns = {"crypto_references": 0, "network_operations": 0, "file_system_operations": 0, "registry_operations": 0, "process_operations": 0, "suspicious_api_calls": 0, "suspicious_score": 0, "detected_patterns": []}
    ...
    def analyze_code_complexity(self, code: str) -> Dict[str, Any]:
        ...
        complexity = {"total_lines": 0, "code_lines": 0, "comment_lines": 0, "blank_lines": 0, "avg_line_length": 0.0, "max_line_length": 0, "cyclomatic_complexity_estimate": 0}
...
class DataProcessor:
    ...
    def _run_global_prefilter(self) -> Tuple[List[Tuple[Path, str]], List[Tuple[Path, str]]]:
        ...
        malicious_raw = [f for f in Path(self.malicious_dir).rglob("*") if f.is_file() and self.js_extractor.is_javascript_file(f)]
        benign_raw = [f for f in Path(self.benign_dir).rglob("*") if f.is_file() and self.js_extractor.is_javascript_file(f)]
        ...
        for f in tqdm(malicious_raw, desc="Prefilter [JS Malicious]"):
            ...
            if not md5:
                logger.warning(f"[JS] Failed to hash malicious file {f}. Moving to problematic_files_js/malicious.")
                self._record_failure("malicious", "prefilter")
                self._move(f, self.problematic_dir / "malicious")
                continue
        ...
        for f in tqdm(benign_raw, desc="Prefilter [JS Benign]"):
            ...
            if not md5:
                logger.warning(f"[JS] Failed to hash benign file {f}. Moving to problematic_files_js/benign.")
                self._record_failure("benign", "prefilter")
                self._move(f, self.problematic_dir / "benign")
                continue
    ...
    def process_dir(self, is_malicious: bool, prefiltered_tasks: List[Tuple[Path, str]]):
        ...
        with ProcessPoolExecutor() as exe:
            for i, feats in enumerate(tqdm(exe.map(self._process_one, tasks), total=total_files, desc=f"Processing JS {label}")):
                f_path = tasks[i][0]
    ...
    def process_dataset(self):
        ...
        logger.info(f"Saved summary to {output_file} ({totals['inserted']:,} inserted, {totals['duplicates']:,} duplicates, {totals['failed']:,} failed)")
...
def main():
    ...
    parser.add_argument("--malicious-dir", default="datamaliciousorder", help="Directory containing malicious JS files")
    parser.add_argument("--benign-dir", default="data2", help="Directory containing benign JS files")
    parser.add_argument("--reset", action="store_true", help="Reset the binary store and index files before processing")
