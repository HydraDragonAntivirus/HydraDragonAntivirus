...
NTSTATUS UserModeUnhookProcess(_In_ ULONG ProcessId)
{
    ...
    // 3. PHASE 1: CLOSE HANDLE
    // Do NOT restore hook bytes here. UserModeUnhookProcess is called because
    // the process is terminating — the OS will reclaim every page in its VA
    // space (including shellcode and the hooked function prologues) automatically.
    //
    // Restoring bytes one hook at a time with UnhookSingleFunction creates a
    // dangerous window: between patching hook N and hook N+1 a thread can execute
    // the now-unhooked function N, which calls function N+1 (still hooked), the
    // shellcode fires and hits a partially-torn-down handle — corrupting state or
    // crashing the target process before it finishes its own cleanup.
    //
    // Correct order: close the handle first (any in-flight shellcode IOCTL will
    // get STATUS_INVALID_HANDLE and bail out cleanly), then detach. Done.
    KeStackAttachProcess((PRKPROCESS)process, &apcState);
    __try
    {
        __try
        ...
        __except (EXCEPTION_EXECUTE_HANDLER)
        {
            DbgPrint("UserModeHook: Exception 0x%X closing handle for PID %lu\n",
