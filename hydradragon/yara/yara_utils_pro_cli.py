...
def clean_yara_file(input_file: str, broken_rules_file: Optional[str] = None, 
                    engine: str = "both", fix_limits: bool = False) -> Dict[str, Any]:
    """
    ...
    if (stats["broken_rules"] > 0 or stats["limit_broken_rules"] > 0 or 
        stats["duplicate_rules"] > 0 or stats["renamed_rules"] > 0):
...
def create_index_file(directory: str, index_file: str, recurse: bool = False):
    ...
    with index_path.open("w", encoding="utf-8") as f:
        f.write("/*\n")
        f.write(f" * YARA-X PRO CLI Generated Index File\n")
        ...
        for file in Path(args.directory).glob(pattern):
            ...
            if "error" not in stats:
                for k in total_stats:
                    if k in stats: total_stats[k] += stats[k]
