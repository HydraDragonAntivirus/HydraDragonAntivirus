...
namespace ProcessUtils
{
    public static class ProcModule
    {
        ...
        public static ModuleInfo[] WinGetModuleInfos(uint processId)
        {
            ...
            GCHandle bufferHandle = new();
            ArrayList moduleInfos = new();
            ...
            try
            {
                ...
                HandleRef handleRef = new(null, handle);
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        internal static extern bool IsWow64Process([In] IntPtr hSourceProcessHandle,
        ...
        private static ModuleInfo[] NtGetModuleInfos(int processId, bool firstModuleOnly)
        {
            ...
            try
            {
                ...
                GCHandle moduleHandlesArrayHandle = new();
                int moduleCount = 0;
                for (; ; )
                {
                        ...
                        if (!enumResult)
                        {
                            ...
                            if (!IsOSOlderThanXP())
                            {
                                ...
                                try
                                {
                                    ...
                                    bool wow64Ret = IsWow64Process(hCurProcess, out sourceProcessIsWow64);
                                    if (!wow64Ret)
                                    {
                                        return null;
                                    }
                ...
                for (int i = 0; i < moduleCount; i++)
                {
                    ...
                    StringBuilder baseName = new(1024);
                    ...
                    StringBuilder fileName = new(1024);
                    ...
                    if (string.Equals(moduleInfo.fileName, "\\SystemRoot\\System32\\smss.exe", StringComparison.OrdinalIgnoreCase))
                    {
                    ...
                    if (moduleInfo.fileName?.Length >= 4
        ...
        public static string PostVerify(uint hprocess, string libFullPath)
        {
            ...
            if (libFullPath?.Length == 0 || !File.Exists(libFullPath))
            {
                return "Invalid file!";
            }
        ...
        public static IntPtr InjectLibraryInternal(uint processid, string libFullPath, out string error)
        {
            ...
            if (modules?.Length > 0)
            {
                ...
                {
                    if (modules[i].baseName.IndexOf("kernel32", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
            ...
            IntPtr hprocess = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_VM_READ | PROCESS_CREATE_THREAD, 0, processid);
            ...
            else
            {
                ...
                IntPtr loadlibraryAddress = (IntPtr)((long)targetkernel.baseOfDll + LoadLibraryArva);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (!GetExitCodeThread(hThread, out IntPtr hLibModule))
                            {
                                VirtualFreeEx(hprocess, pLibPath, 0, FreeType.Release);
                                CloseHandle(hprocess);
        ...
        public static bool FreeLibraryInternal(uint processid, IntPtr libAddress, out string error)
        {
            ...
            else
            {
                ...
                {
                    IntPtr FreeLibAddress = (IntPtr)((long)targetkernel.baseOfDll + FreeLibraryrva);
                    // load dll via call to LoadLibrary using CreateRemoteThread
                    IntPtr hThread = CreateRemoteThread(hprocess, IntPtr.Zero, 0,
    ...
    public static class ExportTable
    {
        ...
        IntPtr hprocess, IntPtr hmodule, string exportnname)
        {
            ...
            int PEOffset = BitConverter.ToInt32(Forread, 0);
            ...
            try
            {
                int ExportDirectoryRVA = BitConverter.ToInt32(Forread, 0);
                int ExportDirectorySize = BitConverter.ToInt32(Forread, 4);
                ...
                if (!ReadProcessMemory(hprocess, (IntPtr)((long)hmodule + IED.AddressOfFunctions), keeper, IED.NumberOfFunctions * 4, ref BytesRead))
                    return -1;

                for (int i = 0; i < funcaddresses.Length; i++)
                    funcaddresses[i] = BitConverter.ToInt32(keeper, i * 4);

                if (!ReadProcessMemory(hprocess, (IntPtr)((long)hmodule + IED.AddressOfNames), keeper, IED.NumberOfNames * 4, ref BytesRead))
                    return -1;

                for (int i = 0; i < funcnames.Length; i++)
                    funcnames[i] = BitConverter.ToInt32(keeper, i * 4);

                if (!ReadProcessMemory(hprocess, (IntPtr)((long)hmodule + IED.AddressOfNameOrdinals), keeper, IED.NumberOfFunctions * 2, ref BytesRead))
                    return -1;
    ...
    public class MemStringReader
    {
        ...
        public static unsafe string ReadUnicodeString(
        IntPtr hprocess, IntPtr address)
        {
            ...
            while (ReadProcessMemory(hprocess, (IntPtr)((long)address + count), datas, 2, ref BytesRead))
            {
        ...
        public static unsafe string ReadAsciiString(
        IntPtr hprocess, IntPtr address)
        {
            if (hprocess == IntPtr.Zero || address == IntPtr.Zero)
                return null;
            ...
            while (ReadProcessMemory(hprocess, (IntPtr)((long)address + count), datas, 1, ref BytesRead))
            {
                if (datas[0] == 0)
                {
                    break;
