#!/usr/bin/env python
"""
OpenHydra Antivirus Engine and API
----------------------------------

This file combines the antivirus engine (dynamic sandbox analysis and static PE analysis)
with a FastAPI REST interface. All actions are logged and all user-facing messages are in English.
The application is designed to be run using uvicorn.

Usage:
    uvicorn openhydra_antivirus:app --host 0.0.0.0 --port=8000

Endpoints:
  - GET / 
      Returns a welcome message and instructions.
  - GET /analyze?mode={mode}&benign_dir={path}&malicious_dir={path}
      Triggers antivirus analysis. 'mode' can be 'dynamic', 'static', or 'both'.

Each analysis function is thoroughly documented inline.
"""
...
class DataProcessor:
    """
    Processes PE files for static analysis.
    """
    ...
    def _move_problematic_file(self, file_path: Path, is_malicious: bool):
        """Moves problematic files to a designated directory."""
    ...
    def process_files(self, directory: str, is_malicious: bool = False) -> Tuple[List[Dict[str, Any]], List[str]]:
        """Processes all files in a directory with duplicate detection."""
        features_list = []
        md5_hashes = set()
        md5_list = []
    ...
    def process_dataset(self):
        """Processes the entire dataset and saves the results."""
        ...
        logging.info(f"Static processing complete. Results saved in {self.output_dir}")

def run_static_analysis(malicious_dir, benign_dir):
    """
    Runs the static analysis process for PE files.
    """
    processor = DataProcessor(malicious_dir, benign_dir)
    processor.process_dataset()

# =============================================================================
# Combined Analysis Dispatcher
# =============================================================================

def combined_main(mode: str, benign_dir: str, malicious_dir: str):
    """
    Dispatches analysis based on the selected mode.
    """
    if mode in ["dynamic", "both"]:
        logging.info("Starting dynamic analysis...")
        run_dynamic_analysis(benign_dir, malicious_dir)
    if mode in ["static", "both"]:
        logging.info("Starting static analysis...")
        run_static_analysis(malicious_dir, benign_dir)
...
app = FastAPI(
    title="OpenHydra Antivirus API",
    description=(
        "This API provides antivirus functionalities using both dynamic (sandbox) and static (PE file) analysis.\n"
        "Each feature is documented with examples and wiki-style explanations.\n"
        "Use the /analyze endpoint to trigger analysis.\n"
        "Parameters:\n"
        "  - mode: 'dynamic', 'static', or 'both'\n"
        "  - benign_dir: path to benign files\n"
...
def read_root():
    ...
    return {"message": "Welcome to OpenHydra Antivirus API. Use /analyze to start an analysis."}

@app.get("/analyze")
def analyze(mode: str = "both", benign_dir: str = "data2", malicious_dir: str = "datamaliciousorder"):
    """
    Triggers antivirus analysis.
    Query parameters:
      - mode: 'dynamic', 'static', or 'both'
      - benign_dir: directory for benign files
      - malicious_dir: directory for malicious files
    """
    try:
        logging.info(f"Analysis request received with mode: {mode}, benign_dir: {benign_dir}, malicious_dir: {malicious_dir}")
        combined_main(mode, benign_dir, malicious_dir)
...
# =============================================================================
# Human-Defined Signature Engine Endpoints (Appended Section)
# =============================================================================

SIGNATURES_FILE = os.path.join(script_dir, "signatures.json")
...
def load_signatures() -> List[Signature]:
    try:
        with open(SIGNATURES_FILE, "r") as f:
            data = json.load(f)
        return [Signature(**sig) for sig in data]
    except Exception as e:
        logging.error(f"Failed to load signatures: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to load signatures: {str(e)}")

def save_signatures(signatures: List[Signature]):
    try:
        with open(SIGNATURES_FILE, "w") as f:
            json.dump([sig.dict() for sig in signatures], f, indent=2)
    except Exception as e:
        logging.error(f"Failed to save signatures: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to save signatures: {str(e)}")

@app.get("/signature", response_model=List[Signature])
def get_signatures():
    """
    Retrieve all human-defined antivirus signatures.
    """
    return load_signatures()

@app.post("/signature", response_model=Signature)
def create_signature(signature: Signature):
    """
    Create a new human-defined antivirus signature.
    """
    signatures = load_signatures()
    # Check for duplicate names (case-insensitive)
    if any(sig.name.lower() == signature.name.lower() for sig in signatures):
        raise HTTPException(status_code=400, detail="A signature with this name already exists.")
    signatures.append(signature)
    save_signatures(signatures)
    logging.info(f"New signature created: {signature.name}")
    return signature
...
if __name__ == "__main__":
    # For direct invocation, run the FastAPI app via uvicorn.
