...
def extract_path_from_marshaled_bytes(
    ...
    if magic_int is not None:
        ...
        # Try to derive magic from the version hint in the data itself
...
def main(argv=None) -> int:
    ...
    if args.version is not None:
        ...
        # Phase 1: collect fast structural hints from up to 512 tag offsets
        ...
        # Phase 2: pick the best window and run a single full xdis probe on it.
        # Choose the offset whose structural hint is the highest version —
        # that window is most likely to contain a representative code object.
        probe_detected = None
        if _hint_offsets:
        ...
        if _all_probe_vers:
            probe_detected = max(_all_probe_vers, key=_xdis_version_sort_key)
        else:
            probe_detected = None
    ...
    try:
        from omni_nuitka_framework import OmniDecompiler, generate_omni_source, generate_omni_nbc, generate_omni_artifact
    except ImportError:
