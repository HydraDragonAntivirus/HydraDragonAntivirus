...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        effect_count = (1 if has_gradient else 0) + shadow_count + len(re.findall(r"backdrop-filter|blur\(", content)) + len(re.findall(r"text-shadow:", content))
        if effect_count > 10:
        ...
        total_animations = len(re.findall(r"@keyframes|transition:|animate-", content)) + (1 if has_lottie else 0) + (1 if has_gsap else 0)
