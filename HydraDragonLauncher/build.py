...
"""
HydraDragon Launcher Build Script

Expected layout:
  HydraDragonAntivirus/
    hydradragon/
      HydraDragonLauncher/                 deploy folder
    HydraDragonLauncher/                   this build.py lives here

Rules:
  - No hardcoded absolute deploy path.
  - Only hydradragonlauncher.exe is copied to deploy.
"""
...
def resolve_layout(script_dir: Path) -> tuple[Path, Path]:
    """
    build.py lives in:
      HydraDragonAntivirus/HydraDragonLauncher/build.py

    Derived:
      repo_root:  HydraDragonAntivirus
      deploy_dir: HydraDragonAntivirus/hydradragon/HydraDragonLauncher
    """
    repo_root = script_dir.parent
    deploy_dir = repo_root / "hydradragon" / script_dir.name
    return repo_root, deploy_dir
...
def build_rust(script_dir: Path, release: bool):
...
def deploy_launcher_only(script_dir: Path, release: bool, deploy_dir: Path):
    step("3/4", "Deploying launcher binary only...")

    target_dir = script_dir / "target" / ("release" if release else "debug")
    robust_copy(target_dir / "hydradragonlauncher.exe", deploy_dir / "hydradragonlauncher.exe")


def main():
    parser = argparse.ArgumentParser(description="HydraDragon Launcher Build Script")
    parser.add_argument("--release", action="store_true", help="Build in release mode")
    parser.add_argument("--run", action="store_true", help="Launch the deployed app after building")
    args = parser.parse_args()

    header("HydraDragon Launcher Build System")

    script_dir = Path(__file__).resolve().parent
    repo_root, deploy_dir = resolve_layout(script_dir)

    check_dependencies()
    build_ui(script_dir, args.release)
    build_rust(script_dir, args.release)
    deploy_launcher_only(script_dir, args.release, deploy_dir)

    target_dir = script_dir / "target" / ("release" if args.release else "debug")
    built_exe = target_dir / "hydradragonlauncher.exe"
    deployed_exe = deploy_dir / "hydradragonlauncher.exe"
    ...
    print(yellow("  to start and manage HydraDragon components."))
    print(cyan("=" * 48))

    if args.run:
        print(magenta("\n  Launching deployed application (UAC prompt may appear)..."))
