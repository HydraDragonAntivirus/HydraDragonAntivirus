...
impl DetectItEasyScanner {
    ...
    fn parse_all_detections(&self, file_path: &Path, die_output: &str) -> DetectItEasyScanResult {
        ...
        let elf_result = file_type_result(
            ...
            validate_elf_file(file_path),
            "Broken Executable",
        );
        let macho_result = file_type_result(
            self.is_macho_file(die_output),
            validate_macho_file(file_path),
            "Broken Executable",
        );
        let apk_result = file_type_result(
            self.is_apk_file(die_output),
            validate_apk_file(file_path),
            "Broken APK",
        );
...
fn file_type_result(detected_by_die: bool, validated: bool, broken_label: &str) -> Option<String> {
    if validated {
...
fn validate_pe_file(file_path: &Path) -> bool {
...
fn validate_elf_file(file_path: &Path) -> bool {
    let Ok(mut file) = File::open(file_path) else {
        return false;
    };
...
fn validate_macho_file(file_path: &Path) -> bool {
    let Ok(mut file) = File::open(file_path) else {
        return false;
    };
...
fn validate_apk_file(file_path: &Path) -> bool {
    let Ok(mut file) = File::open(file_path) else {
        return false;
    };

    let mut magic = [0u8; 4];
    if file.read_exact(&mut magic).is_err() || &magic[0..2] != b"PK" {
        return false;
    }
    ...
    let sample_len = metadata.len().min(64 * 1024 * 1024) as usize;
    let mut data = vec![0u8; sample_len];
    let Ok(read_len) = file.read(&mut data) else {
        return false;
    };
    data.truncate(read_len);

    contains_bytes(&data, b"AndroidManifest.xml") || contains_bytes(&data, b"classes.dex")
}
