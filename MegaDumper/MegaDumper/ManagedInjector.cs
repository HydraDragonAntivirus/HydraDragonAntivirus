...
namespace MegaDumper
{
    ...
    public partial class ManagedInjector : Form
    {
        ...
        private void ManagedInjectorShown(object sender, EventArgs e)
        {
            Text = "Managed injector in " + processname + " whit PID=" + processid.ToString();
        }
        ...
        private static IntPtr CorBindToRuntimeExAddress()
        {
            ...
            if (modules?.Length > 0)
            {
                ...
                {
                    if (modules[i].baseName.IndexOf("mscoree.dll", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
            ...
            if (hprocess != IntPtr.Zero)
            {
                ...
                return (IntPtr)((long)targetmscoree.baseOfDll + CLRCreateInstancerva);
            }
            return IntPtr.Zero;
        }

        private static void WriteUnicodeString(IntPtr Address, string istring)
        {
            UnicodeEncoding Unicode = new();
            byte[] ubytes = Unicode.GetBytes(istring);
            ProcModule.WriteProcessMemory(hprocess, Address, ubytes, (uint)ubytes.Length, out _);
        }
        public static void HostCLR_RunMethod(string AssemblyPath, string TypeName, string MethodName, string Args, string Version)
        {
            ...
            ProcModule.WriteProcessMemory(hprocess, CLSID_CLRRuntimeHostPtr, CLSID_CLRRuntimeHost, (uint)CLSID_CLRRuntimeHost.Length, out _);
            ProcModule.WriteProcessMemory(hprocess, IID_ICLRRuntimeHostPtr, IID_ICLRRuntimeHost, (uint)IID_ICLRRuntimeHost.Length, out _);
            ...
            inline.Retn();
            ProcModule.WriteProcessMemory(hprocess, codeCave_Code, inline.asm, (uint)inline.asm.Length, out _);
            ...
            */
            ...
            if (!ProcModule.GetExitCodeThread(hThread, out _))
            {
                return;
            }
