...
def _generate_heuristic_omni_source(decompiler: OmniDecompiler, section_name: str) -> str:
    ...
    if not _sn_clean:
        _sn_clean = "<section:" + hashlib.sha1(_sn.encode("utf-8", "replace")).hexdigest()[:12] + ">"
    ...
    *,
    ...
    for idx, val in enumerate(constants):
        ...
        if encrypted_cipher_idx is not None and idx == encrypted_cipher_idx:
            continue
        blob_hash = hashlib.sha256(raw[:256]).digest()
...
class OmniNuitkaCompactEmitter:
    ...
    def render(
        ...
        if not _sn_nbc:
            _sn_nbc = "<section:" + hashlib.sha1(_sn_raw.encode("utf-8", "replace")).hexdigest()[:12] + ">"
        out = [
