...
static char *blob_strdup_export(const char *s) {
    size_t n;
    char *d;
    if (!s) return NULL;
    n = strlen(s);
...
static int ensure_dir_exists_export(const char *path) {
    char tmp[4096];
    size_t i, n;
...
static void bytes_to_hex_export(const uint8_t *src, size_t len, char *dst, size_t dst_len) {
...
static int is_ident_piece(const char *s) {
    size_t i;
    if (!s || !*s) return 0;
...
static char *sanitize_module_hint(const char *name) {
    char tmp[512];
    size_t i, j = 0, n;
    int last_dot = 0;
    ...
    tmp[j] = '\0';
    if (j == 0) return NULL;
    return blob_strdup_export(tmp);
}

static char *module_from_py_path_token(const char *token) {
    char buf[512];
    char work[512];
    char *parts[64];
    int part_count = 0;
    char *seg;
    size_t i, n, out_len = 0;
    char *out;
    ...
    /* trim quotes/brackets/punctuation around token */
    while (buf[0] && (buf[0] == '"' || buf[0] == '\'' || buf[0] == '<' || buf[0] == '(' || buf[0] == '['))
        memmove(buf, buf + 1, strlen(buf));
    while ((n = strlen(buf)) > 0 &&
           (buf[n - 1] == '"' || buf[n - 1] == '\'' || buf[n - 1] == '>' || buf[n - 1] == ')' || buf[n - 1] == ']' || buf[n - 1] == ',' || buf[n - 1] == ':'))
        buf[n - 1] = '\0';

    if (!strstr(buf, ".py")) return NULL;

    /* take portion up to .py */
    {
        char *py = strstr(buf, ".py");
        py[3] = '\0';
    }
    ...
    seg = strtok(work, "/");
    while (seg && part_count < 64) {
        if (strcmp(seg, "__pycache__") != 0 && strcmp(seg, ".") != 0 && strcmp(seg, "..") != 0 && is_ident_piece(seg)) {
            parts[part_count++] = seg;
        } else if (strlen(seg) > 3 && strcmp(seg + strlen(seg) - 3, ".py") == 0) {
            char base[256];
            size_t blen = strlen(seg) - 3;
            if (blen >= sizeof(base)) blen = sizeof(base) - 1;
            memcpy(base, seg, blen);
            base[blen] = '\0';
            if (strcmp(base, "__init__") != 0 && is_ident_piece(base)) {
                parts[part_count++] = blob_strdup_export(base); /* temp owned string */
            }
        }
        seg = strtok(NULL, "/");
    }
    ...
    out_len = 0;
    for (i = 0; i < (size_t)part_count; i++) out_len += strlen(parts[i]) + 1;
    out = (char *)malloc(out_len + 1);
    if (!out) return NULL;
    out[0] = '\0';
    for (i = 0; i < (size_t)part_count; i++) {
        if (i) strcat(out, ".");
        strcat(out, parts[i]);
    }

    /* free temp base piece if allocated */
    for (i = 0; i < (size_t)part_count; i++) {
        /* heuristic: only dynamically allocated temp strings won't point inside work/buf */
        if (!(parts[i] >= work && parts[i] < work + sizeof(work))) {
            free(parts[i]);
        }
    }

    return sanitize_module_hint(out);
}

static char *guess_module_name_from_blob(const uint8_t *data, size_t len) {
    size_t i = 0;
    char *best = NULL;
    size_t best_score = 0;
    ...
    while (i < len) {
        if (data[i] >= 0x20 && data[i] <= 0x7E) {
            ...
            size_t run_len;
            char *candidate;
            while (i < len && data[i] >= 0x20 && data[i] <= 0x7E) i++;
            run_len = i - start;
            if (run_len < 4) continue;
            if (run_len >= sizeof(run)) run_len = sizeof(run) - 1;
            memcpy(run, data + start, run_len);
            run[run_len] = '\0';

            candidate = module_from_py_path_token(run);
            if (candidate) {
                size_t score = strlen(candidate);
                if (strchr(candidate, '.')) score += 16;
                if (score > best_score) {
                    free(best);
                    best = candidate;
                    best_score = score;
...
static BlobError write_single_named_bytecode(NamedExportCtx *ctx, const BlobVal *v) {
    char *hint = NULL;
    char file_stub[1024];
    char pyc_path[4096];
    char head_hex[34];
    ...
    hint = guess_module_name_from_blob(v->buf.data, v->buf.len);
    if (hint && *hint) {
        snprintf(file_stub, sizeof(file_stub), "%s__%04u", hint, ctx->next_index);
    } else {
        snprintf(file_stub, sizeof(file_stub), "bytecode_%04u", ctx->next_index);
    }

    snprintf(pyc_path, sizeof(pyc_path), "%s/%s.pyc", ctx->out_dir, file_stub);
    ...
    if (ctx->manifest) {
        ...
        bytes_to_hex_export(v->buf.data, preview_len, head_hex, sizeof(head_hex));
        fprintf(ctx->manifest, "%u\t%s\t%zu\t%s\t%s\n",
                ctx->next_index,
                hint ? hint : "",
                v->buf.len,
                pyc_path,
...
static BlobError export_value_named_bytecode(NamedExportCtx *ctx, const BlobVal *v) {
    size_t i;
    BlobError e;
    if (!v) return BLOB_OK;
    ...
    case BVAL_BYTES:
        if (v->tag == 'X') {
            return write_single_named_bytecode(ctx, v);
        }
        return BLOB_OK;
    case BVAL_TUPLE:
    case BVAL_LIST:
    ...
    case BVAL_RANGE:
        for (i = 0; i < v->seq.count; i++) {
            e = export_value_named_bytecode(ctx, &v->seq.items[i]);
            if (e != BLOB_OK) return e;
        }
        return BLOB_OK;
    case BVAL_DICT:
        for (i = 0; i < v->dval.count; i++) {
            e = export_value_named_bytecode(ctx, &v->dval.keys[i]);
            if (e != BLOB_OK) return e;
            e = export_value_named_bytecode(ctx, &v->dval.vals[i]);
            if (e != BLOB_OK) return e;
        }
        return BLOB_OK;
    default:
...
BlobError blob_export_named_bytecode_files(const BlobVal *vals,
    ...
    if (!vals || !out_dir) return BLOB_ERR_CORRUPT;
    if (!ensure_dir_exists_export(out_dir)) return BLOB_ERR_IO;
    ...
    if (!ctx.manifest || !ctx.pyc_list) {
        if (ctx.manifest) fclose(ctx.manifest);
        if (ctx.pyc_list) fclose(ctx.pyc_list);
        return BLOB_ERR_IO;
    }

    fprintf(ctx.manifest, "index\tmodule_hint\tsize\tpyc_path\tfirst16_hex\n");

    for (i = 0; i < count; i++) {
        e = export_value_named_bytecode(&ctx, &vals[i]);
        if (e != BLOB_OK) {
            fclose(ctx.manifest);
            fclose(ctx.pyc_list);
            return e;
    ...
    if (out_written) *out_written = ctx.written;
    return BLOB_OK;
}
