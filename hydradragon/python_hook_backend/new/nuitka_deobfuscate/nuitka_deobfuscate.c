//     Copyright 2026, Kay Hayen, mailto:kay.hayen@gmail.com find license text
//     at end of file
...
#ifndef _NUITKA_EXPERIMENTAL_DEBUG_CONSTANTS
#define NUITKA_PRINT_TIMING(x)
#endif
#define PRINT_FORMAT(fmt, ...)
#define PRINT_STRING(msg) 
#define NUITKA_CANNOT_GET_HERE(msg) { PyErr_SetString(PyExc_RuntimeError, msg); abort(); }
#define Nuitka_LongFromCLong PyLong_FromLong
#define MAKE_CODE_OBJECT(...) Py_None

#if PY_VERSION_HEX < 0x030c0000
#ifndef Py_INCREF_IMMORTAL
#define Py_INCREF_IMMORTAL(x) Py_INCREF(x)
#endif
#endif

// Prevent Nuitka VLA macros from causing MSVC errors
#ifdef _MSC_VER
#include <malloc.h>
...
#define Nuitka_DictNext PyDict_Next

static PyObject *builtin_module = NULL;
static PyObject *Py_SysVersionInfo = NULL;

static inline PyObject *CALL_FUNCTION_WITH_SINGLE_ARG(PyThreadState *tstate, PyObject *func, PyObject *arg) {
    return PyObject_CallFunctionObjArgs(func, arg, NULL);
}
#define Nuitka_Bytes_FromStringAndSize PyBytes_FromStringAndSize
#define Nuitka_PyLong_FromLong PyLong_FromLong
static inline void Nuitka_LongSetSignNegative(PyObject *long_obj) {
}
#define BUILTIN_COMPLEX2(tstate, r, i) PyComplex_FromDoubles(PyFloat_AsDouble(r), PyFloat_AsDouble(i))
#define PyString_FromStringAndSize PyUnicode_FromStringAndSize
#define CHECK_OBJECT(x) 
#define PyString_InternInPlace PyUnicode_InternInPlace
#define MAKE_SLICE_OBJECT3(tstate, s, t, p) PySlice_New(s, t, p)
#define PyInt_AS_LONG PyLong_AsLong
#define MAKE_XRANGE(tstate, a, b, c) Py_None 
static inline void DECODE(unsigned char* blob) {}
static inline uint32_t calcCRC32(unsigned char const *data, uint32_t size) { return 0; }
#define PyFloat_SET_DOUBLE(obj, val) (((PyFloatObject *)(obj))->ob_fval = (val))

#if PYTHON_VERSION >= 0x030a00
static PyTypeObject *Nuitka_PyUnion_Type = NULL;
#endif

#define NUITKA_STATIC_SMALLINT_VALUE_MIN 0
#define NUITKA_STATIC_SMALLINT_VALUE_MAX 0
#define BUILTIN_XRANGE3(tstate, a, b, c) Py_None
#define MAKE_UNION_TYPE(args) Py_None
#ifndef Py_SET_REFCNT_IMMORTAL
#define Py_SET_REFCNT_IMMORTAL(x)
#endif
#define _PyUnicode_InternImmortal(...) 
#define _PySet_NextEntry(a,b,c,d) 0
...
CONST_CONSTANT unsigned char constant_bin_data[1] = {0};
...
// Use our own non-random hash for some of the things to be fast. This is
// inspired from the original Python2 hash func, but we are mostly using it on
// pointer values
...
static Py_hash_t our_list_hash(PyListObject *list) {
  return Nuitka_FastHashBytes(&list->ob_item[0],
                              Py_SIZE(list) * sizeof(PyObject *));
}

static PyObject *our_list_tp_richcompare(PyListObject *list1,
                                         PyListObject *list2, int op) {
  assert(op == Py_EQ);
  ...
  if (list1 == list2) {
    ...
    result = Py_False;
  } else if (memcmp(&list1->ob_item[0], &list2->ob_item[0],
                    Py_SIZE(list1) * sizeof(PyObject *)) == 0) {
    result = Py_True;
  } else {
    result = Py_False;
  }
...
static Py_hash_t our_tuple_hash(PyTupleObject *tuple) {
  return Nuitka_FastHashBytes(&tuple->ob_item[0],
                              Py_SIZE(tuple) * sizeof(PyObject *));
}

static PyObject *our_tuple_tp_richcompare(PyTupleObject *tuple1,
                                          PyTupleObject *tuple2, int op) {
  assert(op == Py_EQ);
  ...
  if (tuple1 == tuple2) {
    ...
    result = Py_False;
  } else if (memcmp(&tuple1->ob_item[0], &tuple2->ob_item[0],
                    Py_SIZE(tuple1) * sizeof(PyObject *)) == 0) {
    result = Py_True;
  } else {
    result = Py_False;
  }
...
#if PYTHON_VERSION < 0x300
  // Same sized set, simply check if values are identical. Other reductions
  // should make it identical, or else this won't have the effect intended.
...
static PyObject *our_set_tp_richcompare(PyObject *set1, PyObject *set2,
                                        int op) {
  assert(op == Py_EQ);
  ...
  if (Py_SIZE(set1) != Py_SIZE(set2)) {
    ...
    // Same sized dictionary, simply check if values are identical. Other
    // reductions should make it identical, or else this won't have the effect
    // intended.
    while (_PySet_NextEntry(set1, &pos1, &key1, &unused1)) {
      {
        NUITKA_MAY_BE_UNUSED int res =
            _PySet_NextEntry(set2, &pos2, &key2, &unused2);
        assert(res != 0);
      }
...
static PyObject *our_float_tp_richcompare(PyFloatObject *a, PyFloatObject *b,
                                          int op) {
  assert(op == Py_EQ);

  PyObject *result;

  // Avoid the C math when comparing, for it makes too many values equal or
  // unequal.
...
static PyObject *our_dict_tp_richcompare(PyObject *a, PyObject *b, int op) {
  ...
  if (Py_SIZE(a) != Py_SIZE(b)) {
    ...
    while (Nuitka_DictNext(a, &pos1, &key1, &value1)) {
      {
        NUITKA_MAY_BE_UNUSED int res =
            Nuitka_DictNext(b, &pos2, &key2, &value2);
        assert(res != 0);
      }
...
PyObject *Nuitka_Long_SmallValues[NUITKA_STATIC_SMALLINT_VALUE_MAX -
                                  NUITKA_STATIC_SMALLINT_VALUE_MIN + 1];
#endif
#endif
...
#elif PYTHON_VERSION >= 0x300
  for (long i = NUITKA_STATIC_SMALLINT_VALUE_MIN;
       i < NUITKA_STATIC_SMALLINT_VALUE_MAX; i++) {
...
static void insertToDictCacheForcedHash(PyObject *dict, PyObject **value,
                                        hashfunc tp_hash,
...
static PyObject *_Nuitka_Unicode_ImmortalFromStringAndSize(
    PyThreadState *tstate, const char *data, Py_ssize_t size, bool is_ascii) {
#if PYTHON_VERSION < 0x300
  ...
  // TODO: Check with an assertion making build of Python 3.13.0 if this is
  // really true, for 3.14 it ought to not be done.
#if PYTHON_VERSION >= 0x3c0 && PYTHON_VERSION < 0x3e0 &&                       \
    !defined(__NUITKA_NO_ASSERT__)
...
static unsigned char const *_unpackBlobConstants(PyThreadState *tstate,
                                                 PyObject **output,
                                                 unsigned char const *data,
                                                 int count);

static unsigned char const *_unpackBlobConstant(PyThreadState *tstate,
                                                PyObject **output,
                                                unsigned char const *data) {
  ...
  *output = NULL;
  ...
  case 'T': {
    ...
    insertToDictCacheForcedHash(tuple_cache, &t, (hashfunc)our_tuple_hash,
                                (richcmpfunc)our_tuple_tp_richcompare);
  ...
  case 'L': {
    ...
    insertToDictCacheForcedHash(list_cache, &l, (hashfunc)our_list_hash,
                                (richcmpfunc)our_list_tp_richcompare);
  ...
  case 'D': {
    ...
    insertToDictCacheForcedHash(dict_cache, &d, (hashfunc)our_dict_hash,
                                (richcmpfunc)our_dict_tp_richcompare);
  ...
  case 'S': {
    ...
    if (c == 'S') {
      ...
      if (size == 0) {
        ...
        if (empty_frozenset == NULL) {
          empty_frozenset = CALL_FUNCTION_WITH_SINGLE_ARG(
              tstate, (PyObject *)&PyFrozenSet_Type,
    ...
    if (c == 'S') {
      insertToDictCacheForcedHash(set_cache, &s, (hashfunc)our_set_hash,
  ...
  case 'q': {
    ...
    PyObject *l =
        Nuitka_LongFromCLong((c == 'l') ? ((long)value) : (-(long)value));
    assert(l != NULL);

    // Avoid the long cache, won't do anything useful for small ints
#if PYTHON_VERSION >= 0x300
    long check_value = (c == 'l') ? (long)value : -(long)value;
    if (check_value < NUITKA_STATIC_SMALLINT_VALUE_MIN ||
        check_value >= NUITKA_STATIC_SMALLINT_VALUE_MAX)
#endif
    {
  ...
  case 'f': {
    ...
    insertToDictCacheForcedHash(float_cache, &f, NULL,
                                (richcmpfunc)our_float_tp_richcompare);
  ...
  case 'w': {
    ...
    PyObject *u = _Nuitka_Unicode_ImmortalFromStringAndSize(
        tstate, (const char *)data, 1, true);
    data += 1;
  ...
  case 'u': { // Python2 unicode, Python3 str, zero terminated.
    ...
    PyObject *u = _Nuitka_Unicode_ImmortalFromStringAndSize(
        tstate, (const char *)data, size, c == 'a');
    data += size + 1;
  ...
  case 'v': {
    ...
    PyObject *u = _Nuitka_Unicode_ImmortalFromStringAndSize(
        tstate, (const char *)data, size, false);
    data += size;
  ...
  case 's': {
    *output = _Nuitka_Unicode_ImmortalFromStringAndSize(
        tstate, (const char *)data, 0, true);
    is_object = true;
  ...
  case 'E': {
    // Builtin exception by name. TODO: Define number table shared by C and
    // Python serialization to avoid using strings here.
    char const *builtin_exception_name = (char const *)data;
    data = _unpackValueCString(data);

    *output = PyObject_GetAttrString((PyObject *)builtin_module,
                                     builtin_exception_name);
    is_object = true;
  ...
  case 'Z': {
    ...
    case 2: {
      ...
      if (_const_float_plus_nan == NULL) {
        ...
        // Older Python3 has variable signs for NaN from C, so be explicit about
        // it.
        PyFloat_SET_DOUBLE(
            _const_float_plus_nan,
            copysign(PyFloat_AS_DOUBLE(_const_float_plus_nan), 1.0));
      }
    ...
    case 4: {
      ...
      if (_const_float_plus_inf == NULL) {
        ...
        // Older Python3 has variable signs from C, so be explicit about it.
        PyFloat_SET_DOUBLE(
            _const_float_plus_inf,
            copysign(PyFloat_AS_DOUBLE(_const_float_plus_inf), 1.0));
      }
    ...
    // Floats are cached globally too.
    insertToDictCacheForcedHash(float_cache, &z, NULL,
                                (richcmpfunc)our_float_tp_richcompare);
  ...
  case 'C': {
    ...
    *output = (PyObject *)MAKE_CODE_OBJECT(
        Py_None, line_number, co_flags, function_name, function_qualname,
...
static void unpackBlobConstants(PyThreadState *tstate, PyObject **output,
                                unsigned char const *data) {
...
void loadConstantsBlob(PyThreadState *tstate, PyObject **output,
                       char const *name) {
  static bool init_done = false;
  ...
  if (init_done == false) {
    ...
    constant_bin = (const unsigned char *)LockResource(LoadResource(
        handle, FindResource(handle, MAKEINTRESOURCE(3), RT_RCDATA)));
...
#endif
  // Python 3.9 or higher cannot create dictionary before calling init so avoid
  // it.
...
static PyObject* decode_nuitka_blob(PyObject* self, PyObject* args) {
    ...
    if (!Py_SysVersionInfo) {
        ...
        Py_SysVersionInfo = PyObject_GetAttrString(sys, "version_info");
        Py_DECREF(sys);
    }
    ...
    if (blob_size < 8) {
        PyErr_SetString(PyExc_ValueError, "Blob too small");
        return NULL;
    }
    
    // Skip hash and size for now
    const unsigned char* w = blob_data + 8;
    
    PyObject* result_dict = PyDict_New();
    
    while ((Py_ssize_t)(w - blob_data) < blob_size) {
        const char *name = (const char *)w;
        w += strlen(name) + 1;
        uint32_t size;
        memcpy(&size, w, sizeof(uint32_t));
        w += sizeof(uint32_t);
        
        const unsigned char *w_peek = w;
        int count = (int)unpackValueUint16(&w_peek);
        
        PyObject** output_array = (PyObject**)PyMem_Calloc(count, sizeof(PyObject*));
        if (!output_array) {
            PyErr_NoMemory();
        ...
        unpackBlobConstants(tstate, output_array, w);

        PyObject* output_tuple = PyTuple_New(count);
        for(int i=0; i<count; i++) {
            if (output_array[i]) {
                PyTuple_SET_ITEM(output_tuple, i, output_array[i]);
            } else {
                Py_INCREF(Py_None);
                PyTuple_SET_ITEM(output_tuple, i, Py_None);
            }
        }
        PyMem_Free(output_array);
        
        PyDict_SetItemString(result_dict, name, output_tuple);
...
static PyMethodDef BlobLoaderMethods[] = {
    {"decode_blob", decode_nuitka_blob, METH_VARARGS, "Decodes a Nuitka constant blob and returns a dict mapping names to their decoded tuples/lists."},
    {NULL, NULL, 0, NULL}
};
