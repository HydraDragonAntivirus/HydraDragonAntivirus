...
#define NUITKA_DYNAMIC_ARRAY_DECL(name, type, size) type *name = (type *)PyMem_Calloc((size), sizeof(type))
...
static void insertToDictCache(PyObject *dict, PyObject **value) {
  if (!*value) return;
  // Hardening: ONLY cache hashable immutable types to avoid noise and TypeErrors
  if (!PyUnicode_Check(*value) && !PyBytes_Check(*value) && !PyLong_Check(*value) && !PyFloat_Check(*value)) {
      return;
  }
  ...
  if (item != NULL) {
      ...
      *value = item;
      ...
      PyErr_Clear();
      if (PyDict_SetItem(dict, *value, *value) < 0) PyErr_Clear();
  }
}

// Dummy hashes for cache
static Py_hash_t dummy_hash(PyObject *o) { return 0; }
static PyObject* dummy_richcompare(PyObject *a, PyObject *b, int op) { Py_INCREF(Py_NotImplemented); return Py_NotImplemented; }

static void insertToDictCacheForced(PyObject *dict, PyObject **value) {
    insertToDictCache(dict, value);
}

static uint64_t _unpackVariableLength(unsigned char const **data, unsigned char const *end) {
  uint64_t result = 0; uint64_t factor = 1;
  while (*data < end) {
    unsigned char value = **data; *data += 1;
...
static PyObject *_unpackAnonValue(unsigned char anon_index) {
  switch (anon_index) {
    case 0: return (PyObject *)Py_TYPE(Py_None);
    case 1: return (PyObject *)&PyEllipsis_Type;
    case 2: return (PyObject *)Py_TYPE(Py_NotImplemented);
    case 3: return (PyObject *)&PyFunction_Type;
#if PYTHON_VERSION < 0x030000
    case 4: return (PyObject *)&PyGen_Type;
#endif
    case 5: return (PyObject *)&PyCFunction_Type;
    case 6: return (PyObject *)&PyCode_Type;
    case 7: return (PyObject *)&PyModule_Type;
...
static PyObject *_unpackSpecialValue(unsigned char special_index) {
  switch (special_index) {
    case 0: return PyObject_GetAttrString(builtin_module, "Ellipsis");
    case 1: return PyObject_GetAttrString(builtin_module, "NotImplemented");
    case 2: if (Py_SysVersionInfo) { Py_INCREF(Py_SysVersionInfo); return Py_SysVersionInfo; }
    default: Py_INCREF(Py_None); return Py_None;
  }
}
...
static void _unpackBlobConstants(PyThreadState *tstate, PyObject **output, unsigned char const **data, unsigned char const *end, int count, int depth) {
...
static void _unpackBlobConstant(PyThreadState *tstate, PyObject **output, unsigned char const **data, unsigned char const *end, int depth) {
  ...
  *output = NULL;
  ...
  case 'p': {
    ...
    *output = Py_None; Py_INCREF(Py_None); is_object = true; break;
  }
  case 'T':
  case 'L': {
    ...
    if (size > 0 && *data < end) {
      ...
      for (uint32_t i = 0; i < size; i++) {
        PyObject *val = elements[i] ? elements[i] : (Py_INCREF(Py_None), Py_None);
        if (c == 'T') PyTuple_SET_ITEM(coll, i, val);
        else PyList_SET_ITEM(coll, i, val);
      }
      PyMem_Free(elements);
    }
    insertToDictCacheForced((c == 'T') ? tuple_cache : list_cache, &coll);
    *output = coll; is_object = true; break;
  }
  case 'D': {
    uint32_t size = (uint32_t)_unpackVariableLength(data, end);
    ...
    if (size > 0 && *data < end) {
      ...
      for (uint32_t i = 0; i < size; i++) {
        ...
        Py_XDECREF(keys[i]); Py_XDECREF(values[i]);
      }
      PyMem_Free(keys); PyMem_Free(values);
    }
    insertToDictCacheForced(dict_cache, &d);
    *output = d; is_object = true; break;
  }
  case 'P':
  case 'S': {
    uint32_t size = (uint32_t)_unpackVariableLength(data, end);
    ...
    if (size > 0 && *data < end) {
      ...
      for (uint32_t i = 0; i < size; i++) {
        if (elements[i]) {
            if (PySet_Add(s, elements[i]) < 0) PyErr_Clear();
        }
    ...
    *output = s; is_object = true; break;
  }
  case 'i':
  case 'l':
  case 'q': {
    uint64_t val = _unpackVariableLength(data, end);
    *output = PyLong_FromLongLong((c == 'q') ? -(long long)val : (long long)val);
    is_object = true; break;
  }
  case 'f': {
    double fval; memcpy(&fval, *data, sizeof(double)); *data += sizeof(double);
  ...
  case 't': { *output = Py_True; Py_INCREF(Py_True); is_object = true; break; }
  ...
  case 'C': {
    ...
    PyTuple_SET_ITEM(res, 1, arg_names ? arg_names : (Py_INCREF(Py_None), Py_None));
    *output = res; is_object = true; break;
  }
  default: { *output = Py_None; Py_INCREF(Py_None); is_object = true; break; }
  }

  if (is_object && *output) {
#if PYTHON_VERSION < 0x030c00
...
static PyObject* decode_at_offset(PyObject* self, PyObject* args) {
    const unsigned char* blob_data; Py_ssize_t blob_size;
    long long offset;
    if (!PyArg_ParseTuple(args, "y#L", &blob_data, &blob_size, &offset)) return NULL;
...
static PyMethodDef BlobLoaderMethods[] = {
    {"decode_blob", decode_nuitka_blob, METH_VARARGS, "Decodes a Nuitka constant blob."},
