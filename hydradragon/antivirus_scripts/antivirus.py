...
# DetectItEasy results generated by Owlyshield/Rust.
detectiteasy_results_cache: Dict[str, Dict[str, Any]] = {}
...
def cache_detectiteasy_result(file_path: str, detectiteasy_result: Dict[str, Any]) -> None:
    ...
    with detectiteasy_cache_lock:
        detectiteasy_results_cache[file_path.lower()] = detectiteasy_result
        try:
            if "compute_md5" in globals() and os.path.exists(file_path):
                detectiteasy_results_md5_cache[compute_md5(file_path)] = detectiteasy_result
        except Exception:
            pass
    current_detectiteasy_result.set(detectiteasy_result)

def get_cached_detectiteasy_result(file_path: str) -> Optional[Dict[str, Any]]:
    """Get cached DetectItEasy result."""
    with detectiteasy_cache_lock:
        return detectiteasy_results_cache.get(file_path.lower())

def get_detectiteasy_output_from_cache(file_path: str) -> Optional[str]:
    """Get DetectItEasy plain output from the Owlyshield-provided cache."""
    cached = set_current_detectiteasy_result_for_path(file_path)
    if cached:
        return cached.get('die_output')
    return None

def set_current_detectiteasy_result_for_path(file_path: str, file_md5: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """Select the Owlyshield-provided DetectItEasy JSON for this scan context."""
    cached = get_cached_detectiteasy_result(file_path)
    if cached is None:
        try:
            md5 = file_md5
            if md5 is None and "compute_md5" in globals() and os.path.exists(file_path):
                md5 = compute_md5(file_path)
            if md5:
                with detectiteasy_cache_lock:
                    cached = detectiteasy_results_md5_cache.get(md5)
        except Exception:
            cached = None
    if cached:
        current_detectiteasy_result.set(cached)
    return cached
...
def _detectiteasy_result(file_path: Optional[str] = None) -> Optional[Dict[str, Any]]:
    if file_path:
        cached = get_cached_detectiteasy_result(file_path)
        if cached:
            current_detectiteasy_result.set(cached)
        ...
        try:
            if os.path.exists(file_path):
                with detectiteasy_cache_lock:
                    cached = detectiteasy_results_md5_cache.get(compute_md5(file_path))
                if cached:
                    current_detectiteasy_result.set(cached)
                    return cached
        except Exception:
            pass
    return current_detectiteasy_result.get()


logger.debug("DetectItEasy decisions are read directly from Owlyshield Rust JSON.")
...
def analyze_file_with_detectiteasy(file_path):
    """
    Retrieves DetectItEasy output that Owlyshield already produced in Rust.
    """
    try:
        logger.info(f"Retrieving DetectItEasy data for: {file_path} from Owlyshield cache...")

        die_output = get_detectiteasy_output_from_cache(file_path)

        # Display the result using logging
        if die_output and die_output.strip():
            logger.info(f"{'=' * 60}")
            logger.info(f"Cached DetectItEasy Analysis Result for: {Path(file_path).name}")
            logger.info(f"{'=' * 60}")
            logger.info(die_output)
            logger.info(f"{'=' * 60}")
            return die_output
        else:
            logger.warning(f"No cached DetectItEasy output found for {Path(file_path).name}")
            return None

    except Exception as ex:
        error_msg = f"General error in {inspect.currentframe().f_code.co_name} while retrieving DetectItEasy cache for {file_path}: {ex}"
        logger.error(error_msg)
        return None


def get_detectiteasy_output_binary(path: str) -> str:
    """
    Returns DetectItEasy output for a non plain text file, caching by content MD5.
    (Assumes the file isn't plain text, so always calls analyze_file_with_detectiteasy()
     on cache miss.)
    """
    file_md5 = compute_md5(path)
    set_current_detectiteasy_result_for_path(path, file_md5)
    if file_md5 in binary_detectiteasy_cache:
        return binary_detectiteasy_cache[file_md5]

    # First time for this content: use the Owlyshield-provided DetectItEasy result.
    die_output = analyze_file_with_detectiteasy(path)
    binary_detectiteasy_cache[file_md5] = die_output
    return die_output


def get_detectiteasy_output(path: str) -> Tuple[str, bool]:
    """
    Returns (DetectItEasy output, plain_text_flag), caching results by content MD5.
    The plain-text decision is supplied by Owlyshield's Rust DetectItEasy JSON.
    """
    file_md5 = compute_md5(path)
    detectiteasy_result = set_current_detectiteasy_result_for_path(path, file_md5)
    if file_md5 in detectiteasy_cache:
        return detectiteasy_cache[file_md5]

    if detectiteasy_result:
        die_output = detectiteasy_result.get("die_output") or ""
        plain_text_flag = bool(detectiteasy_result.get("is_plain_text"))
    else:
        logger.warning(f"No Owlyshield DetectItEasy JSON available for {Path(path).name}")
        die_output = ""
        plain_text_flag = False

    detectiteasy_cache[file_md5] = (die_output, plain_text_flag)
...
class NuitkaExtractor:
    ...
    def _detect_file_type(self) -> int:
        """Detect the executable file type using Owlyshield's Rust DetectItEasy JSON."""
        get_detectiteasy_output_binary(self.filepath)
        detectiteasy_result = _detectiteasy_result(self.filepath) or {}

        if detectiteasy_result.get("pe_result") == "valid":
            return FileType.PE
        if detectiteasy_result.get("elf_result") == "valid":
            return FileType.ELF
        if detectiteasy_result.get("macho_result") == "valid":
...
def scan_directory_for_executables(directory):
    ...
    def check_file(path):
        get_detectiteasy_output_binary(path)
        detectiteasy_result = _detectiteasy_result(path) or {}
        return detectiteasy_result.get("nuitka_type")
...
def run_jar_extractor(file_path, flag_vineflower):
    ...
    The flag_vineflower indicates if the Owlyshield output also detected a Java class file.
...
async def scan_and_warn(
    ...
    try:
        ...
        # Get DetectItEasy output first (needed for early exit decisions)
        set_current_detectiteasy_result_for_path(norm_path, md5)
        if md5 in detectiteasy_cache:
        ...
        if pefile_result == "valid":
            logger.info(f"The file {norm_path} is a valid PE file.")
            pe_file = True

        apk_result = detectiteasy_result.get("apk_result")
        ...
        elf_result = detectiteasy_result.get("elf_result")
        ...
        macho_result = detectiteasy_result.get("macho_result")
        ...
        async def vmprotect_detection():
            try:
                ...
                if detectiteasy_result.get("is_vmprotect"):
                    try:
                        packed_data = await asyncio.to_thread(Path(file_path).read_bytes)
                        unpacked_data = await asyncio.to_thread(unpack_pe, packed_data)
        ...
        async def themida_detection():
            try:
                is_themida_protected = detectiteasy_result.get("themida_type")
        ...
        async def nexe_thread():
            try:
                if detectiteasy_result.get("is_nexe"):
        ...
        async def autoit_analysis():
            try:
                if detectiteasy_result.get("is_autoit"):
        ...
        async def asar_analysis():
            try:
                if detectiteasy_result.get("is_asar"):
        ...
        async def npm_analysis():
            try:
                if detectiteasy_result.get("is_npm"):
        ...
        async def jsc_analysis():
            ...
            try:
                ...
                jsc_result = detectiteasy_result.get("is_jsc")
                if not jsc_result:
        ...
        async def installshield_analysis():
            try:
                if detectiteasy_result.get("is_installshield"):
        ...
        async def advanced_installer_analysis():
            try:
                if detectiteasy_result.get("is_advanced_installer"):
        ...
        async def clickteam_installer_analysis():
            try:
                if detectiteasy_result.get("is_clickteam"):
        ...
        async def dotnet_analysis():
            try:
                dotnet_result = detectiteasy_result.get("dotnet_type")
        ...
        async def cx_freeze_thread():
            try:
                if detectiteasy_result.get("is_cx_freeze"):
        ...
        if not plain_text_flag:
            ...
            async def upx_thread():
                try:
                    if detectiteasy_result.get("is_upx"):
            ...
            async def protected_process_thread(file_path: str, main_file_path: Optional[str] = None) -> bool:
                """
                Async Task: if a process is running and is marked protected by Rust DetectItEasy JSON,
                ...
                """
                try:
                    protector = detectiteasy_result.get("protector_name")
                    if not protector:
                        return False
            ...
            async def unipacker_thread():
                try:
                    if detectiteasy_result.get("is_packed"):
            ...
            async def inno_setup_thread():
                try:
                    if detectiteasy_result.get("is_inno_setup"):
            ...
            async def go_garble_thread():
                try:
                    if detectiteasy_result.get("is_go_garble"):
            ...
            async def pyc_thread():
                ...
                try:
                    ...
                    if detectiteasy_result.get("is_pyc"):
                        logger.info(f"File {norm_path} is a .pyc file.")

                        # Handle PyArmor-protected .pyc
                        if detectiteasy_result.get("is_pyarmor_archive"):
            ...
            async def nsis_thread():
                try:
                    if detectiteasy_result.get("is_nsis"):
                        nonlocal nsis_flag
                        nsis_flag = True
                except Exception as e:
            ...
            binary_tasks = [
                ...
                asyncio.create_task(protected_process_thread(norm_path, main_file_path)),
        ...
        if pe_file:
            ...
            async def autohotkey_thread():
                ...
                """
                try:
                    if detectiteasy_result.get("is_compiled_autohotkey"):
            ...
            async def enigma1_virtual_box_thread():
                try:
                    if detectiteasy_result.get("is_enigma_virtual_box"):
        ...
        async def jar_analysis_thread():
            try:
                if detectiteasy_result.get("is_jar"):
        ...
        async def java_class_thread():
            try:
                if detectiteasy_result.get("is_java_class"):
        ...
        async def ole2_handler_thread():
            ...
            """
            try:
                if detectiteasy_result.get("is_microsoft_compound"):
        ...
        async def nuitka_thread():
            try:
                nuitka_type = detectiteasy_result.get("nuitka_type")
                if nuitka_type:
        ...
        async def pyinstaller_thread():
            try:
                if detectiteasy_result.get("is_pyinstaller"):
