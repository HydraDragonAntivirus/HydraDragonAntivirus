...
def validate_file(filepath):
    ...
    try:
        ...
        return {"filepath": filepath, "is_pe": is_pe, "hash": file_hash, "size": os.path.getsize(filepath) if is_pe else 0}
    except Exception as e:
        return {"filepath": filepath, "is_pe": False, "hash": None, "error": str(e)}
...
def process_files():
    ...
    if dest.exists():
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            with tqdm(total=len(data2_files), desc="Validating F:\\data2", unit="files", smoothing=0.1) as pbar:
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        with tqdm(total=len(source_files), desc="Validating source", unit="files", smoothing=0.1) as pbar:
            for future in as_completed(futures):
                results = future.result()
    ...
    stats = {"copied": 0, "duplicates": 0, "renamed": 0, "problematic": 0, "errors": 0, "data2_duplicates": moved_duplicates}
