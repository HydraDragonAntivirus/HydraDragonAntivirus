...
//
///
/// @file Network-telemetry-source independent interfaces 
///
...
struct ConnectionInfo
{
	...
	uint64_t nInternalId;   ///< Internal connection id inside the active telemetry source
...
///
/// Compatibility interface for the active network telemetry source.
///
...
public:
	...
	///
	/// Callback on a connection descriptor received directly from an external
	/// telemetry source such as HydraDragonFirewall's named-pipe bridge.
	///
	/// @param pConnectionInfo - parsed connection information.
	///
	virtual void notifyNewConnectionInfo(std::shared_ptr<const ConnectionInfo> pConnectionInfo) = 0;
