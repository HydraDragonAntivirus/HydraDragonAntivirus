...
def clean_yara_file(input_file: str, broken_rules_file: Optional[str] = None, engine: str = "both") -> Dict[str, Any]:
    """
    Cleans a YARA file by:
    1. Sanitizing rule identifiers.
    2. Removing duplicate rules (by name).
    3. Identifying broken rules and moving them to a quarantine file.
    4. Rewriting the original file with only valid, unique rules.
    """
    ...
    broken_blocks: List[Tuple[str, str]] = [] # (rule_name, rule_text)
    seen_identifiers: Set[str] = set()
    
    stats = {
        "original_rules": len(rule_blocks),
        "valid_rules": 0,
        "broken_rules": 0,
        "duplicate_rules": 0,
        "renamed_rules": 0
    ...
    for rule_name, rule_text in rule_blocks:
        ...
        if unique_name != rule_name:
            ...
            rule_text = rule_text.replace(rule_name, unique_name, 1)
        ...
        if is_valid:
            ...
            broken_blocks.append((rule_name, rule_text))
            stats["broken_rules"] += 1
            print(f"Rule '{rule_name}' in {input_file} is broken: {error_msg}")

    # 3. Rewrite original file if changes occurred
    if stats["broken_rules"] > 0 or stats["duplicate_rules"] > 0 or stats["renamed_rules"] > 0:
    ...
    # 4. Write broken rules to quarantine (append mode)
    if broken_blocks:
        with broken_path.open("a", encoding="utf-8") as f:
            f.write(f"// Broken rules extracted on {datetime.datetime.now()} from {input_file}\n")
            for name, text in broken_blocks:
                f.write(f"// Reason: Compilation failed\n")
                f.write(text)
...
def create_index_file(directory: str, index_file: str, recurse: bool = False):
    ...
    with index_path.open("w", encoding="utf-8") as f:
        f.write("/*\n")
        f.write(f" * Generated Index File\n")
...
def compile_yara_x(input_file: str, output_file: str) -> bool:
    ...
    try:
        ...
        # Use yara_x.compile as per user suggestion
        rules = yara_x.compile(source_code)
        
        # Use serialize_into as per user suggestion
        with open(output_file, "wb") as f_out:
            rules.serialize_into(f_out)
        return True
    except yara_x.CompileError as e:
        print(f"Error compiling YARA-X rules: {e}")
        return False
    except Exception as e:
        print(f"Error saving YARA-X rules: {e}")
        return False

# CLI Support (compatible with YARA_Util.py patterns)
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Unified YARA Utility for cleaning, sanitizing, and indexing.")
    ...
    if args.file:
        ...
        stats = clean_yara_file(args.file, broken_rules_file=args.broken, engine=args.engine)
        print(f"Stats: {stats}")
    
    if args.directory:
        print(f"Processing directory: {args.directory}")
        pattern = "**/*.yar*" if args.recurse else "*.yar*"
        total_stats = {"valid_rules": 0, "broken_rules": 0, "duplicate_rules": 0}
        
        for file in Path(args.directory).glob(pattern):
            if file.name.endswith("_broken.yar"): continue
            print(f"  Processing {file}...")
            stats = clean_yara_file(str(file), engine=args.engine)
