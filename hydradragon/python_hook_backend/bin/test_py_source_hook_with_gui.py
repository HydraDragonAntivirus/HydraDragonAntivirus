    ...
    def get_latest_dump_dir(base: str | None = None) -> tuple[str | None, int]:
        ...
        for d in os.listdir(base):
            if d.startswith("dump_"):
                try:
                    candidates.append((int(d.split("_", 1)[1]), os.path.join(base, d)))
                except ValueError:
                    pass
        if not candidates:
            return None, -1
        ...
        while time.time() - start < timeout:
            ...
            if d and idx > baseline_idx:
                if d != seen_dir:
                    ...
                    if log_cb:
                        try:
                            log_cb(f"[ANALYSIS] Detected new dump directory: {d}")
                        except:
                            pass
                ...
                if os.path.exists(started_path) and not announced_start:
                    ...
                    if log_cb:
                        try:
                            log_cb(f"[ANALYSIS] Hook worker started in {d}; waiting for finished.txt...")
                        except:
                            pass
...
class LiteInjector:
    ...
    def log(self, msg):
        ...
        def _bg_log(m):
            try:
                p = r"C:\ProgramData\HydraDragonAntivirus\injector_debug.txt"
    ...
    def _build_ui(self):
        ...
        tk.Label(cfg, text="Hook Script:").pack(anchor="w")
        tk.Entry(cfg, textvariable=self.hook_var).pack(fill=tk.X, pady=2)
        tk.Label(cfg, text="DLL Path:").pack(anchor="w")
        tk.Entry(cfg, textvariable=self.dll_var).pack(fill=tk.X, pady=2)
        ...
        self.log_box = scrolledtext.ScrolledText(self.root, height=10, state="disabled", bg="#1e1e1e", fg="#00ff00", font=("Consolas", 8))
                ...
                if k32.Process32FirstW(h_snap, ctypes.byref(pe)):
                    while True:
                        ...
                        if match or (exe and "python" in exe.lower()):
                            if hide and exe and any(x in exe.lower() for x in ["program files", "windows", "appdata\\local\\programs\\python"]):
                                pass
                            else:
    ...
    def refresh(self):
        try:
            s_term = self.search.get().lower()
            hide = self.hide_std.get()
    ...
    def toggle_ninja(self):
        ...
        if self.ninja_on:
            self.root.after(100, self.ninja_loop)

    def ninja_loop(self):
        if not self.ninja_on:
            return
                    ...
                    while True:
                        ...
                        if pid not in self.processed and pid != os.getpid():
                            ...
                            if self.is_target_native(pid, name, exe):
                                ...
                                threading.Thread(target=self.inject, args=(pid, name, exe)).start()
    ...
    def is_target_native(self, pid, name, exe):
        n = name.lower()
        e = exe.lower()
        if any(x in n for x in ["csrss", "lsass", "services", "wininit", "winlogon", "smss", "fontdrvhost", "svchost", "explorer", "taskhost"]):
            return False
        if self.hide_std.get() and any(x in e for x in ["program files", "windows", "appdata\\local\\programs\\python"]):
            return False
    ...
    def _create_remote_thread(self, h_proc, start, param, pid, label):
        ...
        h = k32.CreateRemoteThread(h_proc, None, 0, start, param, 0, ctypes.byref(tid))
        if h:
            return h
        h_nt = wintypes.HANDLE()
        status = ntdll.NtCreateThreadEx(ctypes.byref(h_nt), 0x1FFFFF, None, h_proc, start, param, 0, 0, 0, 0, None)
    ...
    def _wait_thread_exit(self, h, timeout, label):
        if k32.WaitForSingleObject(h, timeout) == 0:
            ec = wintypes.DWORD()
            k32.GetExitCodeThread(h, ctypes.byref(ec))
    ...
    def run_inject(self):
        ...
        threading.Thread(target=self.inject, args=(int(pid), name, exe)).start()
    ...
    def inject(self, pid, name, exe):
        ...
        try:
            ...
            if pid == os.getpid():
                self.log(f"ERROR: Refusing to inject into own process (PID {pid}).")
                return
            ...
            if not py_dll:
                self.log(f"ERROR: No python3.dll / python3XX.dll loaded in PID {pid} ({name}). Not a Python process.")
                return
            ...
            try:
                ...
                self.log(f"ERROR: FAILED TO WRITE CONFIG: {e}")
                return
            ...
            if self._find_remote_module_base(pid, os.path.basename(dll_path)):
                self.log(f"[*] {os.path.basename(dll_path)} already loaded in PID {pid} — skipping re-inject.")
                return
            ...
            if not h_proc:
                ...
                return
            try:
                p_bytes = (dll_path + "\0").encode("utf-16le")
                mem = k32.VirtualAllocEx(h_proc, 0, len(p_bytes), 0x3000, 0x40)
                if not mem:
                    self.log(f"ERROR: VirtualAllocEx FAILED: {k32.GetLastError()}")
                    return
                ...
                ok, l_res = self._wait_thread_exit(h_load, 5000, "Load")
                k32.CloseHandle(h_load)
                if not ok or not l_res:
                    self.log(f"ERROR: LoadLibraryW FAILED. Res: {l_res}, Error: {k32.GetLastError()}")
                    return
                rem_mod = self._find_remote_module_base(pid, os.path.basename(dll_path))
                if not rem_mod:
                    if l_res and l_res != 0:
                        rem_mod = l_res
                        self.log(f"[*] Module base from thread code: 0x{rem_mod:X}")
                    else:
                        self.log(f"ERROR: Module not found in remote process: {os.path.basename(dll_path)}")
                        return
                try:
                    if not pefile:
                        self.log("ERROR: pefile MISSING. Using fallback export.")
                        fn_rva = 0x1000
                    else:
                        pe = pefile.PE(dll_path)
                        fn_rva = None
                        for export in pe.DIRECTORY_ENTRY_EXPORT.symbols:
                            if export.name == b"HydraStartHook":
                                fn_rva = export.address
                                break
                        if not fn_rva:
                            self.log("ERROR: HydraStartHook EXPORT NOT FOUND.")
                            return
                except Exception as pe_err:
                    self.log(f"ERROR: pefile failed: {pe_err}")
                    return
                ...
                rem_fn = int(rem_mod) + fn_rva
                h_st = self._create_remote_thread(h_proc, rem_fn, None, pid, "Start")
                if not h_st:
                    self.log("ERROR: CreateRemoteThread Start FAILED.")
                    return
                ok, exit_code = self._wait_thread_exit(h_st, 5000, "Start")
                k32.CloseHandle(h_st)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not shell32.IsUserAnAdmin():
            shell32.ShellExecuteW(None, "runas", sys.executable, f'"{os.path.abspath(__file__)}"', None, 1)
            sys.exit()
        root = tk.Tk()
        app = LiteInjector(root)
        root.mainloop()
    except Exception as e:
        import tkinter.messagebox as mbox

        try:
            mbox.showerror("Fatal Error", f"Failed to launch: {e}")
        except:
