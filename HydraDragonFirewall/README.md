...
- **Signature-First Filtering**: Built-in whitelist feeds are removed; every packaged threat feed (including prior "whitelist" CSVs) is treated as a blocking signature so only configured allow rules or per-app approvals can open traffic.
