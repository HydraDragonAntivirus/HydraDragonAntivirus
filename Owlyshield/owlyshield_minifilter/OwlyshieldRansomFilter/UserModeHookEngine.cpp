...
// =============================================================================
// DEFAULT API HOOK TABLE
// =============================================================================
//
// Called once from UserModeHookEngineInitialize.  Pre-registers every function
// we always want to intercept so that ANY process that gets hooked immediately
// starts reporting real API calls — not just DLL-load file-I/O heuristics.
//
// EventId ranges:
//   0x6100–0x611F  ws2_32  (sockets)
//   0x6120–0x612F  winhttp
//   0x6130–0x613F  wininet
//   0x6140–0x614F  urlmon / dnsapi
//   0x6200–0x620F  kernel32 – process/thread
//   0x6300–0x631F  kernel32 / ntdll – memory/injection
//   0x6320–0x633F  ntdll – native NT
//   0x6400–0x640F  advapi32 – registry
//   0x6500–0x650F  shell32 / kernel32 – execution/load
//   0x6600–0x660F  bcrypt / crypt32
// =============================================================================
static VOID RegisterDefaultApiHooks(VOID)
{
// Helper: stack-allocate a HOOK_CONFIG_DATA, fill it, hand it to AddCustomHook.
// FunctionName is CHAR[], ModuleName is WCHAR[].
#define REG_HOOK(wmod, afunc, evid)                                         \
    do {                                                                     \
        HOOK_CONFIG_DATA _cfg;                                               \
        RtlZeroMemory(&_cfg, sizeof(_cfg));                                  \
        (VOID)RtlStringCchCopyW(_cfg.ModuleName,                             \
                                RTL_NUMBER_OF(_cfg.ModuleName),              \
                                (wmod));                                     \
        (VOID)RtlStringCchCopyA(_cfg.FunctionName,                           \
                                RTL_NUMBER_OF(_cfg.FunctionName),            \
                                (afunc));                                    \
        _cfg.EventId = (evid);                                               \
        (VOID)AddCustomHook(&_cfg);                                          \
    } while (0)

    // ── ws2_32 – raw socket API ───────────────────────────────────────────
    REG_HOOK(L"ws2_32.dll", "WSAStartup",       0x6100);
    REG_HOOK(L"ws2_32.dll", "WSACleanup",       0x6101);
    REG_HOOK(L"ws2_32.dll", "socket",           0x6102);
    REG_HOOK(L"ws2_32.dll", "connect",          0x6103);
    REG_HOOK(L"ws2_32.dll", "bind",             0x6104);
    REG_HOOK(L"ws2_32.dll", "listen",           0x6105);
    REG_HOOK(L"ws2_32.dll", "accept",           0x6106);
    REG_HOOK(L"ws2_32.dll", "send",             0x6107);
    REG_HOOK(L"ws2_32.dll", "recv",             0x6108);
    REG_HOOK(L"ws2_32.dll", "sendto",           0x6109);
    REG_HOOK(L"ws2_32.dll", "recvfrom",         0x610A);
    REG_HOOK(L"ws2_32.dll", "closesocket",      0x610B);
    REG_HOOK(L"ws2_32.dll", "WSASocketW",       0x610C);
    REG_HOOK(L"ws2_32.dll", "WSAConnect",       0x610D);
    REG_HOOK(L"ws2_32.dll", "WSASend",          0x610E);
    REG_HOOK(L"ws2_32.dll", "WSARecv",          0x610F);
    REG_HOOK(L"ws2_32.dll", "WSASendTo",        0x6110);
    REG_HOOK(L"ws2_32.dll", "WSARecvFrom",      0x6111);
    REG_HOOK(L"ws2_32.dll", "getaddrinfo",      0x6112);
    REG_HOOK(L"ws2_32.dll", "GetAddrInfoW",     0x6113);
    REG_HOOK(L"ws2_32.dll", "gethostbyname",    0x6114);

    // ── winhttp ───────────────────────────────────────────────────────────
    REG_HOOK(L"winhttp.dll", "WinHttpOpen",             0x6120);
    REG_HOOK(L"winhttp.dll", "WinHttpConnect",          0x6121);
    REG_HOOK(L"winhttp.dll", "WinHttpOpenRequest",      0x6122);
    REG_HOOK(L"winhttp.dll", "WinHttpSendRequest",      0x6123);
    REG_HOOK(L"winhttp.dll", "WinHttpReceiveResponse",  0x6124);
    REG_HOOK(L"winhttp.dll", "WinHttpReadData",         0x6125);
    REG_HOOK(L"winhttp.dll", "WinHttpWriteData",        0x6126);

    // ── wininet ───────────────────────────────────────────────────────────
    REG_HOOK(L"wininet.dll", "InternetOpenA",           0x6130);
    REG_HOOK(L"wininet.dll", "InternetOpenW",           0x6131);
    REG_HOOK(L"wininet.dll", "InternetConnectA",        0x6132);
    REG_HOOK(L"wininet.dll", "InternetConnectW",        0x6133);
    REG_HOOK(L"wininet.dll", "InternetOpenUrlA",        0x6134);
    REG_HOOK(L"wininet.dll", "InternetOpenUrlW",        0x6135);
    REG_HOOK(L"wininet.dll", "HttpSendRequestA",        0x6136);
    REG_HOOK(L"wininet.dll", "HttpSendRequestW",        0x6137);
    REG_HOOK(L"wininet.dll", "InternetReadFile",        0x6138);
    REG_HOOK(L"wininet.dll", "InternetWriteFile",       0x6139);

    // ── urlmon + dns ──────────────────────────────────────────────────────
    REG_HOOK(L"urlmon.dll",  "URLDownloadToFileA",         0x6140);
    REG_HOOK(L"urlmon.dll",  "URLDownloadToFileW",         0x6141);
    REG_HOOK(L"urlmon.dll",  "URLDownloadToCacheFileA",    0x6142);
    REG_HOOK(L"urlmon.dll",  "URLDownloadToCacheFileW",    0x6143);
    REG_HOOK(L"dnsapi.dll",  "DnsQuery_A",                 0x6144);
    REG_HOOK(L"dnsapi.dll",  "DnsQuery_W",                 0x6145);

    // ── kernel32 – process / thread ───────────────────────────────────────
    REG_HOOK(L"kernel32.dll", "CreateProcessA",         0x6200);
    REG_HOOK(L"kernel32.dll", "CreateProcessW",         0x6201);
    REG_HOOK(L"kernel32.dll", "OpenProcess",            0x6202);
    REG_HOOK(L"kernel32.dll", "TerminateProcess",       0x6203);
    REG_HOOK(L"kernel32.dll", "CreateThread",           0x6204);
    REG_HOOK(L"kernel32.dll", "CreateRemoteThread",     0x6205);
    REG_HOOK(L"kernel32.dll", "CreateRemoteThreadEx",   0x6206);
    REG_HOOK(L"kernel32.dll", "SuspendThread",          0x6207);
    REG_HOOK(L"kernel32.dll", "ResumeThread",           0x6208);

    // ── kernel32 / ntdll – memory / injection ─────────────────────────────
    REG_HOOK(L"kernel32.dll", "VirtualAlloc",           0x6300);
    REG_HOOK(L"kernel32.dll", "VirtualAllocEx",         0x6301);
    REG_HOOK(L"kernel32.dll", "VirtualProtect",         0x6302);
    REG_HOOK(L"kernel32.dll", "VirtualProtectEx",       0x6303);
    REG_HOOK(L"kernel32.dll", "WriteProcessMemory",     0x6304);
    REG_HOOK(L"kernel32.dll", "ReadProcessMemory",      0x6305);
    REG_HOOK(L"kernel32.dll", "MapViewOfFile",          0x6306);
    REG_HOOK(L"kernel32.dll", "CreateFileMappingA",     0x6307);
    REG_HOOK(L"kernel32.dll", "CreateFileMappingW",     0x6308);
    REG_HOOK(L"kernel32.dll", "QueueUserAPC",           0x6309);

    // ── ntdll – native NT layer ───────────────────────────────────────────
    REG_HOOK(L"ntdll.dll", "NtAllocateVirtualMemory",   0x6320);
    REG_HOOK(L"ntdll.dll", "NtWriteVirtualMemory",      0x6321);
    REG_HOOK(L"ntdll.dll", "NtReadVirtualMemory",       0x6322);
    REG_HOOK(L"ntdll.dll", "NtProtectVirtualMemory",    0x6323);
    REG_HOOK(L"ntdll.dll", "NtMapViewOfSection",        0x6324);
    REG_HOOK(L"ntdll.dll", "NtCreateSection",           0x6325);
    REG_HOOK(L"ntdll.dll", "NtCreateProcess",           0x6326);
    REG_HOOK(L"ntdll.dll", "NtCreateProcessEx",         0x6327);
    REG_HOOK(L"ntdll.dll", "NtCreateThreadEx",          0x6328);
    REG_HOOK(L"ntdll.dll", "NtOpenProcess",             0x6329);
    REG_HOOK(L"ntdll.dll", "NtSuspendProcess",          0x632A);
    REG_HOOK(L"ntdll.dll", "NtTerminateProcess",        0x632B);
    REG_HOOK(L"ntdll.dll", "NtQueueApcThread",          0x632C);
    REG_HOOK(L"ntdll.dll", "NtSetInformationProcess",   0x632D);
    REG_HOOK(L"ntdll.dll", "NtCreateFile",              0x632E);
    REG_HOOK(L"ntdll.dll", "NtWriteFile",               0x632F);
    REG_HOOK(L"ntdll.dll", "NtReadFile",                0x6330);
    REG_HOOK(L"ntdll.dll", "NtDeleteFile",              0x6331);

    // ── advapi32 – registry ───────────────────────────────────────────────
    REG_HOOK(L"advapi32.dll", "RegCreateKeyExA",        0x6400);
    REG_HOOK(L"advapi32.dll", "RegCreateKeyExW",        0x6401);
    REG_HOOK(L"advapi32.dll", "RegSetValueExA",         0x6402);
    REG_HOOK(L"advapi32.dll", "RegSetValueExW",         0x6403);
    REG_HOOK(L"advapi32.dll", "RegDeleteKeyA",          0x6404);
    REG_HOOK(L"advapi32.dll", "RegDeleteKeyW",          0x6405);
    REG_HOOK(L"advapi32.dll", "RegDeleteValueA",        0x6406);
    REG_HOOK(L"advapi32.dll", "RegDeleteValueW",        0x6407);
    REG_HOOK(L"advapi32.dll", "RegOpenKeyExA",          0x6408);
    REG_HOOK(L"advapi32.dll", "RegOpenKeyExW",          0x6409);

    // ── shell32 / kernel32 – execution + library loading ─────────────────
    REG_HOOK(L"shell32.dll",  "ShellExecuteA",          0x6500);
    REG_HOOK(L"shell32.dll",  "ShellExecuteW",          0x6501);
    REG_HOOK(L"shell32.dll",  "ShellExecuteExA",        0x6502);
    REG_HOOK(L"shell32.dll",  "ShellExecuteExW",        0x6503);
    REG_HOOK(L"kernel32.dll", "LoadLibraryA",           0x6504);
    REG_HOOK(L"kernel32.dll", "LoadLibraryW",           0x6505);
    REG_HOOK(L"kernel32.dll", "LoadLibraryExA",         0x6506);
    REG_HOOK(L"kernel32.dll", "LoadLibraryExW",         0x6507);
    REG_HOOK(L"kernel32.dll", "GetProcAddress",         0x6508);
    REG_HOOK(L"kernel32.dll", "WinExec",                0x6509);

    // ── bcrypt / crypt32 – crypto ─────────────────────────────────────────
    REG_HOOK(L"bcrypt.dll",  "BCryptEncrypt",           0x6600);
    REG_HOOK(L"bcrypt.dll",  "BCryptDecrypt",           0x6601);
    REG_HOOK(L"bcrypt.dll",  "BCryptGenerateSymmetricKey", 0x6602);
    REG_HOOK(L"bcrypt.dll",  "BCryptGenRandom",         0x6603);
    REG_HOOK(L"crypt32.dll", "CryptEncrypt",            0x6610);
    REG_HOOK(L"crypt32.dll", "CryptDecrypt",            0x6611);
    REG_HOOK(L"crypt32.dll", "CryptImportKey",          0x6612);
    REG_HOOK(L"crypt32.dll", "CryptGenRandom",          0x6613);
    REG_HOOK(L"crypt32.dll", "CryptDeriveKey",          0x6614);

#undef REG_HOOK

    DbgPrint("!!! UserModeHook: Registered %lu default API hooks.\n", g_CustomHookCount);
}
...
NTSTATUS UserModeHookEngineInitialize(VOID)
{
    ...
    // Register the built-in hook catalog so every new process attachment
    // automatically covers real API calls, not just DLL-load heuristics.
    RegisterDefaultApiHooks();
