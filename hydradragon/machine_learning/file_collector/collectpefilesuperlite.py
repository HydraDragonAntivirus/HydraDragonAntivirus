...
# Setup logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s", handlers=[logging.FileHandler("pe_scanner.log"), logging.StreamHandler()])
...
def worker_scan_file(args):
    ...
    try:
        ...
        return {"path": path, "size_mb": round(size / (1024 * 1024), 2), "md5": md5}
    except (IOError, OSError) as e:
...
def load_existing_hashes_parallel(folder):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        results = list(tqdm(executor.map(worker_load_hash, files, chunksize=50), total=len(files), unit="file", desc="Hashing"))
            ...
            for batch_num in range(start_batch, total_batches):
                ...
                for future in tqdm(as_completed(futures), total=len(futures), unit="file", desc=f"Batch {batch_num + 1}/{total_batches}"):
                    try:
                        ...
                        logger.warning("Worker timeout - file took too long")
                        continue
                    except Exception as e:
...
def save_cache(hashes, filename=CACHE_FILE):
    ...
    try:
        with open(filename, "w") as f:
            json.dump({"hashes": list(hashes), "timestamp": time.time(), "count": len(hashes)}, f, indent=2)
