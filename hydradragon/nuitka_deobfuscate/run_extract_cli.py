...
class PBKDF2AESBypass:
    ...
    def try_decrypt(self, encrypted_payload: bytes, password: bytes,
        ...
        Expected payload format (from reverse-engineered Nuitka Commercial stub):
            [4 bytes: IV seed as uint32-LE]
            [rest: AES-CBC encrypted data]
        ...
        iv_seed = struct.unpack('<I', data[:4])[0]
            ...
            if PBKDF2AESBypass.detect_pbkdf2_pattern(constants):
                ...
                if enc_payload:
                    ...
                    if salt:
                        ...
                        print(f"    [!] No salt found in constants")
                    ...
                    for pw in pw_candidates:
                        ...
                        if not nbc_consts:
                            print(f"    [.] wrong password, trying next")
                            continue
                        ...
                        if not nbc_code_objs:
                            print(f"    [.] no code objects — false positive, skipping")
                            continue
                        ...
                        for _co in nbc_code_objs:
                            try:
                                ...
                                print(f"    [x] _dump_code_object failed for code obj")
                        if payloads:
                    ...
                    if mod_name not in _pbkdf2_recovered:
                        ...
                        for pw in path_pw_candidates:
                            print(f"    [-] Trying path-based password {pw!r} for {mod_name}...")
                            decrypted = _pbkdf2_bypass.try_decrypt(enc_payload, pw, salt=salt)
                            print(f"    [.] decrypt result: {len(decrypted) if decrypted else 'None'} bytes")
                            if not decrypted or len(decrypted) <= 16:
                                print(f"    [x] too short or None")
