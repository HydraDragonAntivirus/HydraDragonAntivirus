...
#if IS_DEBUG_IRP
_LOGINFO_RAW("!!! Regedit: Failed to enqueue registry event for PID %lu\n",
                 newEntry->data.PID);
#endif
...
NTSTATUS RegistryCallback(_In_ PVOID CallbackContext, _In_ PVOID Argument1, _In_ PVOID Argument2)
{
    ...
    RegPath.Buffer = (PWCH)ExAllocatePoolWithTag(NonPagedPool, REGPATH_MAX_BYTES, REG_TAG);
...
#if IS_DEBUG_IRP
_LOGINFO_RAW("RegPath append failed: buffer too small\n");
#endif
...
#if IS_DEBUG_IRP
_LOGINFO_RAW("!!! Regedit: exception in RegistryCallback (class=%d, PID=%lu)\n",
...
#if IS_DEBUG_IRP
_LOGINFO_RAW("[Registry-Protection] Initialized successfully\r\n");
#endif
